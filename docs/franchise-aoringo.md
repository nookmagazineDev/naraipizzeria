...
| หน้าย่อย | เห็นอะไร |
|---|---|
| รายงานยอดขาย | ตารางรายบิล พร้อมปุ่ม **ดูบิล** (หัวบิล + รายการในบิล + วิธีชำระ) และ **ประวัติออเดอร์** (ทุกความเคลื่อนไหวของออเดอร์นั้นจากตาราง `OrderActivity`) — ทรงเดียวกับ "รายงานยอดการขาย" ของเมนู ACC |
| แดชบอร์ด | ยอดขายรวม · จำนวนบิล · จำนวนลูกค้า · รายจ่าย · กราฟยอดขายรายวัน · ช่องทางชำระเงิน · เมนูขายดี 10 อันดับ · รายจ่ายตามประเภท |
| ยอดขายรายวัน | ตารางทรงเดียวกับ "ยอดรายวัน" ของเมนู ACC: Dine-in/Take-Home/Delivery · Service Charge · ส่วนลด · Net/Vat/Gross · แยกช่องทางชำระ · Total Sales · บิล/หัว/เฉลี่ย · รายจ่ายของวันนั้น (กดหัวคอลัมน์เพื่อเรียง · กดตัวเลขเพื่อดูบิลเบื้องหลัง) |
...
| ประวัติออเดอร์ | `OrderActivity` (ชนิดความเคลื่อนไหว · รายละเอียด · เวลา · คนทำรายการ) — ไม่มีวันที่ของบิลในตัวเอง ต้อง join `SaleOrder` เหมือนตารางรายการสินค้า |

### ประวัติออเดอร์ (`OrderActivity`)

ปุ่ม **ประวัติออเดอร์** ในหน้า "รายงานยอดขาย" (และในหน้าต่าง "ดูบิล") เปิดไทม์ไลน์ของออเดอร์นั้น
— เปิดออร์เดอร์ / สั่งเพิ่ม / แก้ไข / ยกเลิก / ชำระเงิน เรียงเก่า→ใหม่ พร้อมคนที่ทำรายการ

ชื่อคอลัมน์ของตารางนี้ต่างกันได้ตามเวอร์ชันของแอป จึงเลือก "ตัวแรกที่มีจริง" ให้ตอนรัน:

| ช่องที่หน้าเว็บใช้ | คอลัมน์ที่ลองหา |
|---|---|
| อ้างถึงออเดอร์ | `OrderId` · `SaleOrderId` (ไม่มีสักตัว = อ่านประวัติไม่ได้) |
| เวลาที่เกิดเหตุ | `ActivityAt` · `CreatedAt` · `ActivityDate` · `OccurredAt` · `LoggedAt` · `ActivityTime` · `CreateDate` (ไม่มีเลย → ใช้เวลาของบิล) |
| ชนิดความเคลื่อนไหว | `ActivityType` · `Action` · `ActionType` · `EventType` · `Activity` · `Event` · `Type` · `Status` |
| รายละเอียด | `Description` · `Detail` · `Details` · `Message` · `Note` · `Remark` · `Content` · `Data` |
| คนทำรายการ | join `AppUser` ด้วย `UserId`/`AppUserId`/`CreatedByUserId`/… ไม่มี FK ก็ใช้ `UserName`/`CreatedBy` ตรง ๆ |
| เมนู/จำนวน/ยอด (มีก็โชว์) | `MenuName`·`ItemName` · `Qty`·`Quantity` · `Amount`·`Total`·`Value` |

กรองช่วงวันที่ด้วย **วันที่ของบิล** (`COALESCE(PaidAt, OrderDate, OpenedAt)`) ไม่ใช่เวลาของความเคลื่อนไหว
— ทุกบิลที่ขึ้นบนหน้าจอจะได้มีประวัติของตัวเองครบ ไม่ตกหล่นที่หัว/ท้ายช่วง

ตารางที่ชื่อไม่ตรง (`SaleOrderActivity` · `OrderActivityLog` · `OrderLog`) ก็รับได้
ฐานไหนไม่มีตารางนี้เลย หน้าอื่นยังใช้ได้ตามปกติ — หน้าต่างประวัติจะขึ้นเหตุผลที่อ่านไม่ได้แทน
...
(ประวัติออเดอร์มีเฉพาะโหมด `app` เท่านั้น โหมดนี้ไม่มีให้ — ตารางแบบ log ไม่มีคอลัมน์ตายตัวพอจะเดาได้)
...
| ทาง | endpoint |
|---|---|
| Vercel | `/api/franchise?view=all\|sales\|detail\|expense\|activity\|schema\|diag&start=YYYY-MM-DD&end=YYYY-MM-DD` |
| host | `/aoringo/ping` · `/aoringo/schema` · `/aoringo/sales` · `/aoringo/detail` · `/aoringo/expense` · `/aoringo/activity` |
