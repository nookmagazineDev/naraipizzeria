...
| หน้าย่อย | เห็นอะไร |
|---|---|
| รายงานยอดขาย | ปุ่มด้านบน: **ใบกำกับภาษีอย่างย่อ (ABB)** = ออกทุกบิลในช่วงที่เลือก เรียงตามเวลา · **ใบกำกับภาษีเต็มรูปแบบ** = เลือกบิล กรอกข้อมูลผู้ซื้อ แล้วพิมพ์ A4 (มีจำนวนเงินเป็นตัวอักษร) · ข้อมูลร้านบนหัวเอกสารกรอกครั้งเดียว เครื่องจำไว้ให้ · ตารางรายบิล พร้อมปุ่ม **ดูบิล** (หัวบิล + รายการในบิล + วิธีชำระ) และ **ประวัติออเดอร์** (ทุกความเคลื่อนไหวของออเดอร์นั้นจากตาราง `OrderActivity`) — ทรงเดียวกับ "รายงานยอดการขาย" ของเมนู ACC |
...
## ใบกำกับภาษี (หน้ารายงานยอดขาย)

- **อย่างย่อ (ABB)** — ร้านที่จด VAT ออกให้ลูกค้าทุกบิลอยู่แล้ว หน้านี้จึงออกให้ *ทุกบิล* ในช่วงวันที่ที่โหลดไว้
  เรียงตามเวลา (บิลที่ยกเลิกไม่ออกให้) · แสดงได้สูงสุด 500 ใบต่อครั้ง มากกว่านั้นให้แคบช่วงวันที่ลง
- **เต็มรูปแบบ** — เลือกบิลที่ลูกค้าขอ แล้วกรอก ชื่อ/เลขประจำตัวผู้เสียภาษี/ที่อยู่/สาขา ของผู้ซื้อก่อนพิมพ์
- ภาษีเอาค่าจาก `SaleOrder.VatAmount` เป็นหลัก ฐานไหนไม่ได้เก็บไว้จะถอดออกจากยอดรวมให้เอง
  ตามอัตราใน `VatRate` (ไม่มีก็ใช้ 7%) โดยถือว่าราคาขายรวมภาษีไว้แล้ว — แบบที่ร้านอาหารใช้กัน
- ข้อมูลร้านบนหัวเอกสาร (ชื่อ/ที่อยู่/เลขผู้เสียภาษี/สาขา/โทร) เก็บไว้ใน localStorage ของเครื่องที่ใช้งาน
  ยังไม่ได้ดึงจากตาราง `Setting` ในฐาน — ใครเปิดจากเครื่องใหม่ต้องกรอกครั้งแรกครั้งเดียว
- พิมพ์เป็น A4 แนวตั้งด้วย named page `@page tax` ใน `styles/globals.css` (คนละแนวกับรายงานเงินเดือน
  ซึ่งเป็นแนวนอน) ตอนสั่งพิมพ์จะใส่คลาส `fc-print-mode` ที่ body เพื่อซ่อนทุกอย่างนอกกล่องเอกสาร
