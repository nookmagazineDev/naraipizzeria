...
> พนักงานไปอยู่ฐาน `narai_hr` ตัวเดียวกับตารางงาน/กะ ไม่ใช่ `InventoryNarai` เหมือนชุดอื่น — ดูข้อ 2.1
...
| ชีทพนักงาน แท็บ `DATA` | รายชื่อพนักงาน | `narai_hr.dbo.hr_employee` (คนละฐาน — ดูข้อ 2.1) |

รายละเอียดคอลัมน์และเหตุผลที่เลือกคีย์แบบนั้น อยู่ในคอมเมนต์ของ [`schema-sheets.sql`](schema-sheets.sql)

### 2.1) พนักงานไปอยู่ฐาน `narai_hr` ไม่ใช่ `InventoryNarai`

ตอนย้ายรอบแรกสร้าง `InventoryNarai.dbo.hr_employee` ขึ้นมาใหม่จากชีท แต่บนเครื่อง SQL ตัวเดียวกัน
มี `narai_hr.dbo.hr_employee` อยู่ก่อนแล้ว — เป็นตารางที่ **ตารางงาน/กะ ของ Narai-branch ใช้อยู่**
(`dbo.hr_timesheet` อ้าง `hr_code` ของฐานนี้) กลายเป็นรายชื่อพนักงานนอนอยู่สองที่ ถูกแก้กันคนละรอบ
ชื่อ/สาขา/ตำแหน่ง/สถานะจึงเพี้ยนกันไปเรื่อย ๆ โดยไม่มีอะไรเตือน

รวมเหลือตารางเดียวที่ **`narai_hr`** เพราะประวัติกะผูกกับ `hr_code` ของฐานนั้นอยู่แล้ว ย้ายกลับทางอื่นไม่ได้

* สคริปต์รวมข้อมูล (รันครั้งเดียวที่เครื่องออฟฟิศ ด้วย SSMS): [`schema-hr-employee.sql`](schema-hr-employee.sql)
  — เติมคอลัมน์ที่ `narai_hr` ไม่เคยมี (`start_date`, `loga`, `new_code`, `photo_url`, `sort_order`,
  `updated_at`) แล้วย้ายข้อมูลจาก `InventoryNarai` มารวม รันซ้ำได้ไม่พัง
* ชื่อตารางที่โค้ดใช้อยู่ที่เดียว: `HR_EMPLOYEE_TABLE` ใน `lib/sheetsSql.mjs`
  (ตั้ง env `HR_EMPLOYEE_TABLE` ทับได้ทั้งฝั่งเครื่องโฮสต์และ Vercel)
* `InventoryNarai.dbo.hr_employee` เหลือไว้เป็นสำเนาสำรอง **ไม่มีใครอ่าน/เขียนแล้ว** ไม่ต้องลบ
* **ต้องให้สิทธิ์ login ที่เว็บใช้ในฐาน `narai_hr` ด้วย** — สิทธิ์ที่ให้ไว้ตอน `schema-sheets.sql`
  ครอบแค่ฐาน `InventoryNarai` ไม่ให้จะขึ้นว่า `is not able to access the database`
  (ท่อน `GRANT` อยู่ท้าย `schema-hr-employee.sql`)
* ด้วยเหตุนี้ **ตัวย้ายข้อมูลจากชีทไม่มีชุด `employee` แล้ว** ทั้ง `scripts/migrate-sheets.mjs`
  และ `/api/sheets-migrate` — ตารางจริงถูกแก้จากหน้าเว็บทุกวัน ย้ายจากชีททับซ้ำอีกจะเอาข้อมูลเก่า
  กลับมาทับของจริง
...
| env | ค่า | ไว้ทำอะไร |
|---|---|---|
| `SHEETS_SOURCE` | `sheet` (เริ่มต้น) / `sql` | สลับให้หน้าเว็บอ่าน-เขียน SQL แทนชีท (**ไม่คุมส่วนพนักงาน** — พนักงานยึด SQL เสมอ) |
...
| `/api/stock-gas` `getEmployees` / `saveEmployee` | ส่งต่อไป Apps Script | ทำกับ `narai_hr.dbo.hr_employee` ตรง ๆ **เสมอ** (ไม่ขึ้นกับ `SHEETS_SOURCE`) |
...
* **ยกเว้นพนักงาน** (`getEmployees` / `saveEmployee` และเครื่องมือ AI `get_employees_summary`)
  — ยึด `narai_hr.dbo.hr_employee` ที่เดียว **ไม่ถอยไปชีททั้งอ่านและเขียน** ต่อฐานไม่ได้จะขึ้น error ให้เห็นเลย
  เพราะถ้าอ่านชีทแต่เขียนฐาน อาการที่คนใช้เจอคือ "กดบันทึกขึ้นสำเร็จ แต่ข้อมูลไม่เปลี่ยน"
...
อ่านกลับมาเป็นอีกที่ที่ไม่ได้ถูกเขียน — ปิดทางนี้ไปแล้วด้วยการให้พนักงาน **ยึด SQL อย่างเดียว**

| ปิดยังไง | ที่ไฟล์ |
|---|---|
| `getEmployees` / `saveEmployee` ไม่ดู `SHEETS_SOURCE` และไม่ถอยไปชีท ต่อฐานไม่ได้ = ขึ้น error | `pages/api/stock-gas.js` |
| เครื่องมือ AI `get_employees_summary` อ่านตารางเดียวกับหน้ารายชื่อ ไม่ถอยไป GAS | `pages/api/ai-chat.js` |
...
|---|---|---|
| `host API ตอบไม่ใช่ JSON (HTTP 404)` | host-server ที่ออฟฟิศเป็นเวอร์ชันเก่าที่ยังไม่มี `/sheets/*` | ที่เครื่องออฟฟิศ `git pull` แล้ว `start-narai.ps1 -Restart` (ต้องปิด node ตัวเก่า ไม่งั้นโค้ดใหม่ไม่ถูกโหลด) |
| `โหมด SQL ยังเขียนไม่ได้ — ตั้ง …` | Vercel ไม่มีกุญแจฝั่งเขียน | ตั้ง `SHEETS_WRITE_KEY` บน Vercel ให้ตรงกับเครื่องโฮสต์ |
| `Invalid object name 'narai_hr.dbo.hr_employee'` | ยังไม่ได้รันสคริปต์รวมรายชื่อที่เครื่องออฟฟิศ | รัน `docs/schema-hr-employee.sql` ด้วย SSMS |
| `… is not able to access the database "narai_hr"` | login ที่เว็บใช้ยังไม่มีสิทธิ์ในฐานนั้น | รันท่อน `GRANT` ท้าย `docs/schema-hr-employee.sql` |
| `ไม่พบพนักงานรหัส … ในฐานข้อมูล` | ตารางมีข้อมูล แต่ไม่มีรหัสนั้น | เช็กว่าย้ายข้อมูลครบไหม (`/sheets/ping` ดูจำนวนแถว) |

ถ้าบันทึกแล้วขึ้น error ให้ดูข้อความในวงเล็บว่าติดตรงไหน (ต่อ host API ไม่ได้ / ไม่มีกุญแจเขียน /
ไม่พบรหัสในตาราง) แล้วแก้ตามนั้น — ระหว่างนั้นข้อมูลจะไม่ถูกเขียนลงที่ไหนเลย ไม่มีสองที่ให้ตามแก้ทีหลัง

> ชีทพนักงานยังอยู่ครบเป็นต้นทางของการย้ายข้อมูล และ `employee-apps-script.gs` ยังเก็บไว้
> เผื่อต้องกลับไปดูของเก่า — แต่หน้าเว็บไม่ยิงไปหามันแล้ว และไม่มีตัวย้ายจากชีทให้เรียกอีก
> (แก้ที่ชีทตอนนี้จะไม่ไปถึงหน้าเว็บ ต้องแก้ที่หน้ารายชื่อพนักงานเท่านั้น)
