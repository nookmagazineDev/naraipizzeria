...
> ⚠️ **อัปเดต ส.ค. 2026 — แพลนสินค้าไม่ได้อ่านสำเนาแล้ว** `/api/plan` อ่านใบสั่งสดจาก
> MySQL `myfbdata.orderd` (ฐานเดียวกับแอปสั่งของของสาขา) ส่วน `dbo.stock_plan` กับชีท `plan`
> เหลือเป็นทางถอยเวลาต่อ POS ไม่ได้ — **ดูข้อ 6** ก่อนเชื่อตารางในข้อ 2 กับข้อ 5
...
|---|---|---|
| สต๊อก `1xegMu…` แท็บ `plan` | จัดซื้อ > แพลนสินค้า (**ทางถอยแล้ว** ดูข้อ 6) | `dbo.stock_plan` |
...
| ทาง | ก่อน | หลัง |
|---|---|---|
| `/api/plan` | อ่านชีท `plan` | อ่าน `dbo.stock_plan` (**ตั้งแต่ ส.ค. 2026 เป็นแค่ทางถอย** ดูข้อ 6) |
...
## 6) แพลนสินค้าเลิกอ่านสำเนาแล้ว — อ่านใบสั่งสดจาก POS (ส.ค. 2026)

ตารางข้างบนยังจริงสำหรับ `stock_closing` / `expense_*` แต่ **แพลนสินค้าเดินคนละทางแล้ว**

ปัญหาของทางเดิม: ทั้งชีท `plan` และ `dbo.stock_plan` เป็น **สำเนา** ที่ต้องมีคนรันคัดลอก
ของที่สาขาเพิ่งกดสั่งจึงไม่ขึ้นจนกว่าจะมีคนรัน — เป็นที่มาของอาการ "ไม่มีข้อมูลวันล่าสุด"

ทางใหม่: `/api/plan` อ่านตรงจาก **MySQL `myfbdata.orderd`** ที่ `inventory.dyndns.tv`
ซึ่งเป็นตารางเดียวกับที่แอปสั่งของของสาขา (`narai-storefct`) เขียนลง ข้อมูลจึงตรงกันทันทีที่สาขากดสั่ง
โค้ดอยู่ที่ [`lib/planLive.js`](../lib/planLive.js) ใช้ env ชุดเดียวกับ `pages/api/orderd.js` (ตั้งบน Vercel ไว้แล้ว)

| ลำดับ | แหล่ง | หมายเหตุ |
|---|---|---|
| 1 (ทางหลัก) | `myfbdata.orderd` ผ่าน MySQL | ข้อมูลสด `source: 'pos'` |
| 2 (ถอย) | `dbo.stock_plan` | แนบ `warning` ว่าข้อมูลอาจไม่สด |
| 3 (ถอย) | ชีท `plan` | แนบ `warning` เหมือนกัน |

⚠️ `orderd` มี ~2.2 ล้านแถวและไม่มี index ตามสาขา/วันที่ — **ต้องส่งช่วงวันที่มาเสมอ**
`GET /api/plan?from=YYYY-MM-DD&to=YYYY-MM-DD` (ไม่ส่ง = 30 วันล่าสุด) และตั้ง `maxDuration: 60`
ไว้เพราะคิวรีกวาดตารางใหญ่ใช้เวลาได้ถึง ~10 วิ ซึ่งยาวกว่า default ของ Vercel

### ทะเบียนไอเทม "หมวดแพลน"

หน้าเว็บดึงใบสั่ง**ทุกใบ**มาก่อน แล้วค่อยคัดเฉพาะรหัสที่อยู่ในหมวดแพลน
รายชื่อนั้นเก็บที่ตาราง `InventoryNarai.dbo.plan_item` แก้ได้จากหน้าเว็บเอง
(จัดซื้อ > แพลนสินค้า > จัดการรายการ) ไม่ต้อง deploy

* สคีมา + เหตุผลการออกแบบ: [`schema-plan-item.sql`](schema-plan-item.sql)
  (กดปุ่ม "สร้างตาราง" ในหน้าจัดการรายการก็ได้ — login ต้องมีสิทธิ์ `db_ddladmin`)
* API: `pages/api/plan-items.js` — ต่อฐานด้วย pool เดียวกับ QC/RD และทะเบียนสาขา (`lib/qcrdPool.js`)
* ต่อฐานไม่ได้/ยังไม่ได้สร้างตาราง → ถอยไปใช้ `PLAN_ITEMS` ใน [`lib/planItems.js`](../lib/planItems.js)
  แล้วแนบ `warning` **ไม่ใช่** ตอบ error ทิ้งหน้าให้ว่าง
* รหัสสินค้าเก็บแบบตัด 0 นำหน้าออกเสมอ (`normItemCode`) เพราะฝั่ง POS เขียนมาไม่เหมือนกันทุกแถว

## 7) ชีทที่ยัง **ไม่ได้** ย้ายในรอบนี้
...
| `lib/planLive.js` | อ่านใบสั่งสดจาก `myfbdata.orderd` — ทางหลักของแพลนสินค้า (ข้อ 6) |
| `lib/planItems.js` | รายการตั้งต้น/สำรองของหมวดแพลน + ตัวจับคู่รหัส (`normItemCode`) |
| `pages/api/plan-items.js` | ทะเบียนหมวดแพลน `dbo.plan_item` (อ่าน/เพิ่ม/ลบ/สร้างตาราง) |
| `docs/schema-plan-item.sql` | โครงตาราง `dbo.plan_item` (รันซ้ำได้) |
