...
## รอบเดือน — ปิดใน 2 วันสุดท้าย นับเป็นรอบของเดือนถัดไป

ที่ร้านนับสต๊อกปิดรอบกันวันท้าย ๆ ของเดือน แล้วถือว่ายอดชุดนั้นเป็นรอบของเดือนที่กำลังจะขึ้น
ถ้าจัดกลุ่มตามเดือนของ `closing_date` ตรง ๆ รอบเดียวกันจะถูกแยกเป็นสองเดือนทันทีที่
บางสาขานับ 31 ส.ค. และบางสาขานับ 1 ก.ย.

กติกา: **รอบเดือน = เดือนของ (วันที่ปิดยอด + 2 วัน)** — เขียนทีเดียวจบ ไม่ต้องเช็กว่าเดือนไหนมีกี่วัน
...
| 26 ก.พ. | กุมภาพันธ์ |
| 27–28 ก.พ. | **มีนาคม** |
| 31 ธ.ค. | มกราคมปีถัดไป |

- ฝั่ง SQL ใช้ `CONVERT(char(7), DATEADD(day, 2, closing_date), 126)` ฝั่ง JS ใช้ `cycleMonth()`
  ทั้งคู่อ่านค่าจาก `CYCLE_SHIFT_DAYS` ใน `lib/monthEndSql.mjs` — จะเปลี่ยนเป็น 1 หรือ 3 วันสุดท้าย
  แก้เลขตัวเดียวตรงนั้น ทั้ง dropdown เลือกเดือน · ตัวกรอง · หน้าสรุป เปลี่ยนตามหมด
- ป้าย "ตามหลังรอบล่าสุด" ในหน้าสรุปเทียบด้วย**รอบเดือน** ไม่ใช่วันที่ — สาขาที่นับ 31 ส.ค.
  กับ 1 ก.ย. อยู่รอบเดียวกัน ไม่มีใครตามหลังใคร
- ถ้าคอลัมน์วันที่เป็น**ข้อความ** (ไม่ใช่ชนิด date) จะเลื่อนวันไม่ได้ ระบบถอยไปจัดกลุ่มตามเดือน
  ของวันที่ปิดยอดตรง ๆ และหน้าเว็บจะไม่ขึ้นหมายเหตุกติกานี้ (`layout.cycleShiftDays = 0`)
