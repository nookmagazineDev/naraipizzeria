...
คือ **"รอบสิงหาคม"** ชุดเดียวกัน — จัดกลุ่มตามเดือนของ `closing_date` ตรง ๆ จะได้สองรอบทันที
...
| **1–2 ก.ย.** | **สิงหาคม** |
| 3 ก.ย. | กันยายน |
| **1–2 มี.ค.** | **กุมภาพันธ์** (ปีอธิกสุรทินก็ถูก) |
| 1 ม.ค. | ธันวาคมปีก่อน |

- ฝั่ง SQL ใช้ `CONVERT(char(7), DATEADD(day, -2, closing_date), 126)` ฝั่ง JS ใช้ `cycleMonth()`
  ทั้งคู่อ่านค่าจาก `CYCLE_SHIFT_DAYS` ใน `lib/monthEndSql.mjs` — เปลี่ยนเป็น −1 / −3 (หรือค่าบวก
  ถ้าจะให้วันท้ายเดือนกลายเป็นรอบเดือนถัดไปแทน) แก้เลขตัวเดียวตรงนั้น ทั้ง dropdown เลือกเดือน ·
  ตัวกรอง · หน้าสรุป · ข้อความหมายเหตุบนหน้าเว็บ เปลี่ยนตามหมด
