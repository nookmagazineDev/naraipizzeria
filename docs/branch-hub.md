...
> | 5 | ย้าย login สาขาจากชีทเข้าฐาน แล้วตั้ง `SHEET_LOGIN_URL=off` — **ไม่ทำข้อนี้ การตั้งรหัสใหม่จะถูกย้อนกลับเงียบ ๆ** (หัวข้อ 9) | คนที่เปิดชีท User ได้ |
>
...
>
> ✅ เรื่อง HPS ปิดจบแล้ว: ยืนยันว่า **902 ถูก** — แก้ในโค้ดทุกที่และมีขั้นซ่อมข้อมูลในไฟล์ย้ายแล้ว (หัวข้อ 8)
...
|---|---|---|---|
| **HPS** | `109` ❌ | `902` ✅ | **ขัดกันตรง ๆ** — ยืนยันแล้วว่า 902 ถูก แปลว่าฝั่งนี้ (และระบบตารางงาน) อ่านยอดผิดร้านมาตลอด |
...
1. ✅ เคาะแล้วว่า **HPS = 902** — แก้ในโค้ดทุกที่แล้ว ส่วนแถวที่ลงฐานไปแล้วซ่อมด้วยขั้นที่ 4 ของ `migrate-branch-hub.sql`
...
### เรื่อง HPS: ยืนยันแล้วว่า 902 ถูก (ก.ย. 2026)

แปลว่า **109 ผิดมาตลอด** และทุกที่ที่ใช้ 109 อ่านยอดของ HPS ผิดร้าน
ฝั่งเดียวที่ถูกอยู่แล้วคือ `narai-storefct`

จุดที่ต้องแก้มีมากกว่าที่คิด — ระบบตารางงานถือตารางแมปฝังไว้เองอีก 5 ที่:

| ที่ | แก้แล้ว |
|---|---|
| `naraipizzeria/lib/branchCore.mjs` (รายชื่อสำรอง) | ✅ |
| `naraipizzeria/docs/schema-hr-branch.sql` (seed) | ✅ |
| แถวที่ลงฐานไปแล้ว | ✅ ขั้นที่ 4 ของ `migrate-branch-hub.sql` |
| `Narai-branch/api/orderd.js` · `api/usage.js` · `api/withdrawals.js` | ✅ |
| `Narai-branch/office-server/server.js` | ✅ |
| `Narai-branch/apps-script.js` | ✅ ในรีโป — **ต้อง deploy Apps Script ใหม่ถึงจะมีผล** |
| `narai-storefct/lib/branches.js` | ถูกอยู่แล้ว |

ห้าที่ในระบบตารางงานเป็นตารางแมปที่ฝังไว้เอง ไม่ได้อ่านจากทะเบียนกลาง — เป็นงานที่ควรทำต่อ
(เหมือนที่โปรเจคนี้เลิกใช้ตาราง `OUTLETS` ไปแล้ว) แต่ยังไม่ได้ทำในรอบนี้

**หลักฐานที่ยืนยัน 902:** `Narai-branch/api/insert_order.js` กับ `api/pending_orders.js`
ถือตารางกลับด้าน (outlet → ชื่อฐานสาขา) ที่เขียน `902: 'hps'` อยู่แล้ว และคอมเมนต์เหนือตารางนั้น
ระบุว่า *"ตรวจสอบแล้วจากค่า Cfg_LstOrdID ที่ตรงกับใบสั่งล่าสุดของแต่ละสาขาจริง"*
— คือมีคนเทียบกับข้อมูล POS จริงมาแล้ว ในโปรเจคเดียวกันจึงมีสองที่พูดคนละอย่าง

### ⬜ ค้าง: FCT (950) เป็นสาขาไหม

ตารางกลับด้านชุดเดียวกันนั้นมี `950: 'fct'` อยู่ด้วย ซึ่ง**ไม่เคยอยู่ในทะเบียนสาขาเลย**
เดาว่าเป็นครัวกลาง/โรงงาน ไม่ใช่สาขาขายหน้าร้าน จึงยังไม่เพิ่มเข้าทะเบียน —
ต่างจาก STS/ZK3 ที่ยืนยันแล้วว่าเป็นสาขาที่ปิดไป

ถ้า FCT ควรอยู่ในทะเบียนด้วย ต้องเคาะก่อนว่า **เปิดอยู่หรือปิดแล้ว** เพราะถ้าเปิดอยู่
จะโผล่ใน dropdown เลือกสาขาของทุกหน้าทันที (ต่างจาก STS/ZK3 ที่เพิ่มแบบปิดไว้)
...
### ⚠️ ทางถอยไปถามชีท ทำให้การตั้งรหัสใหม่ถูกย้อนกลับได้

ลำดับตรวจตอนสาขาล็อกอิน (`Narai-branch/office-server/schedule.js`):

```
หาใน hr_user → รหัสตรง?  → เข้าได้
                รหัสไม่ตรง → ถามชีทต่อ → ชีทผ่าน → upsertUser() เขียนทับ hr_user
                                                    ด้วยรหัส "และสาขา/outlet id" จากชีท
```

แปลว่าถ้าตั้งรหัสใหม่จากหน้า `HR → login สาขา` แล้วพนักงานยังพิมพ์รหัสเก่าที่อยู่ในชีท
ระบบจะ **ย้อนรหัสกลับเป็นของเก่าให้เอง** และทับสาขาที่เพิ่งแก้ไปด้วย โดยไม่มี error
ไม่มีเตือน — เห็นแต่อาการ "ตั้งรหัสไปแล้วทำไมรหัสเก่ายังใช้ได้"

ที่ **ไม่** โดนย้อนคือการปิดการใช้งาน — โค้ดเช็ก `is_active` ก่อนถึงชั้นชีทแล้วโยน error ทันที

### ย้าย login จากชีทเข้าฐานให้ครบ แล้วปิดทางถอย

ปิดทางถอยได้ด้วย `SHEET_LOGIN_URL=off` ใน `.env` ของ office-server
แต่ปิดก่อนที่ทุกคนจะมีแถวใน `hr_user` = สาขานั้นล็อกอินไม่ได้ทันที จึงต้องย้ายให้ครบก่อน

`scripts/import-hr-users.mjs` ทำส่วนนั้น:
...
**ทำไมต้อง export ไฟล์เอง ไม่ให้สคริปต์ดึงจากชีทตรง ๆ** — Apps Script ของชีทไม่มี action ไหน
ที่คืนรายชื่อผู้ใช้ออกมา (มีแค่ `login` ที่ตรวจทีละคู่) ส่วนการอ่านผ่าน gviz ต้องตั้งชีทเป็น
"ผู้ที่มีลิงก์ • ผู้อ่าน" ก่อน ซึ่งเท่ากับเอารหัสของทุกสาขาไปวางไว้กลางแจ้ง
(เหตุผลเดียวกับที่ office-server เลือกย้ายทีละคนตอนล็อกอินแทนการเขียนสคริปต์ย้าย)

ให้คนที่เปิดชีทได้อยู่แล้ว export เป็น CSV ลงเครื่องออฟฟิศ แล้วรันสคริปต์ที่เครื่องนั้น
รหัสจึงไม่เคยวิ่งออกเน็ต และไม่ต้องเปิดสิทธิ์ชีทให้ใครเพิ่ม

พฤติกรรมที่ตั้งใจให้เป็นแบบนี้:

| กรณี | ทำอะไร |
|---|---|
| ไม่ใส่ `--commit` | ดูอย่างเดียว ไม่เขียนฐาน |
| บัญชีมีอยู่แล้วในฐาน | **ข้าม** — ถ้าเคยตั้งรหัสใหม่จากหน้าเว็บไปแล้ว การทับจะย้อนกลับเป็นรหัสเก่าในชีท (`--overwrite` ถ้าตั้งใจจริง ๆ) |
| รหัสในชีทสั้นกว่าเกณฑ์ใหม่ | **นำเข้าให้ตามเดิม** แล้วรายงานรายชื่อ — ไม่ให้ใครล็อกอินไม่ได้เพราะกติกาที่เพิ่งตั้ง |
| ชื่อผู้ใช้ซ้ำในไฟล์ / ไม่มีรหัส / ไม่มีสาขา | ข้ามแล้วบอกเลขบรรทัด |

ลำดับทั้งหมด: export CSV → รันดูก่อน → รัน `--commit` → เทียบที่หน้า `HR → login สาขา`
ว่าบัญชีครบทุกสาขาที่ยังเปิดอยู่ → ตั้ง `SHEET_LOGIN_URL=off` แล้วรีสตาร์ท office-server
→ **ลบไฟล์ CSV ทิ้ง** (มีรหัสผ่านของทุกสาขาเป็นข้อความล้วน)
