...
| ทาง | เมื่อไหร่ |
|---|---|
| อัตโนมัติ | ทุกครั้งที่บันทึกจากหน้า QC/RD สำเร็จ — ดันเฉพาะชุดที่เกี่ยว (แก้วัตถุดิบ → `item`, แก้สูตร → `menu`+`bom`) |
| ปุ่ม **อัพขึ้น SQL** | หน้า QC/RD > วัตถุดิบ — ดันทะเบียนวัตถุดิบทั้งชีท แล้วบอกจำนวนแถวชีท ↔ SQL |
| เปิดเอง | `POST /api/qcrd-sync` `{ "steps": "item" }` หรือ `GET /api/qcrd-sync?steps=all&verify=1` |

ดันด้วย `MERGE` (เพิ่ม/อัปเดต) ทำซ้ำกี่รอบก็ได้ ข้อมูลไม่ซ้ำ
...
| เรื่อง | เป็นยังไง |
|---|---|
| **ลบแถวไม่ตามขึ้นไป** | `MERGE` ไม่ลบแถวที่หายไปจากชีท — ลบวัตถุดิบในชีทแล้ว `stock_item` ยังมีอยู่ หน้านับสต๊อกจึงยังเห็น **ให้ตั้งสถานะเป็น "ปิดการใช้งาน" แทนการลบ** (หน้านับสต๊อกกรองสถานะนี้ออกอยู่แล้ว) |
