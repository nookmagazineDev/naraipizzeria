...
export { cycleMonth, CYCLE_PREV_MONTH_UNTIL_DAY, normalizeId };
...
/** 'YYYY-MM' -> 'รอบสิงหาคม 2569' (ปี พ.ศ. ตามที่ออฟฟิศเรียกกัน — เหมือนหน้าปิดรอบเดือน) */
...
export function dateLabel(iso) {
  ...
  return `${Number(m[3])} ${(TH_MONTHS[Number(m[2]) - 1] || '').slice(0, 3)}. ${String(Number(m[1]) + 543).slice(-2)}`;
}
