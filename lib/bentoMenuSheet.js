...
// ว่าง = แท็บแรกของชีท — ห้ามตั้งค่าเริ่มต้นเป็น '0' เพราะแท็บแรกไม่จำเป็นต้องมี gid=0
// (แท็บที่สร้างใหม่/แท็บแรกเดิมถูกลบ ได้ gid สุ่ม) และขอ gid ที่ไม่มีอยู่ Google ตอบ HTTP 400
const BENTO_SHEET_GID = env('BENTO_SHEET_GID', '');
...
/**
 * อ่านชีทเป็น CSV — ลอง export ก่อน (ค่าดิบตรงตามชีท) ไม่ได้ค่อยถอยไป gviz
 * gviz เดาชนิดคอลัมน์แล้วอาจทิ้งค่าบางช่อง แต่ยอมรับชีทที่ export ปฏิเสธได้มากกว่า
 */
async function fetchCsv() {
  const base = `https://docs.google.com/spreadsheets/d/${BENTO_SHEET_ID}`;
  const gid = BENTO_SHEET_GID ? `&gid=${encodeURIComponent(BENTO_SHEET_GID)}` : '';
  const urls = [`${base}/export?format=csv${gid}`, `${base}/gviz/tq?tqx=out:csv${gid}`];
  const errors = [];
  for (const url of urls) {
    try {
      ...
      // ชีทที่ไม่ได้แชร์ Google ส่งหน้า login กลับมาด้วย HTTP 200
      if (/^\s*<(!doctype|html)/i.test(text)) { errors.push('ได้หน้าเว็บแทน CSV'); continue; }
      return text;
    } catch (err) {
      errors.push(err.message);
    }
  }
  throw new Error(`อ่านชีทข้าวกล่องไม่ได้ (Google Sheets ${errors.join(' / ')})`
    + (BENTO_SHEET_GID ? ` — ตรวจว่าแท็บ gid=${BENTO_SHEET_GID} มีอยู่จริง` : '')
    + ' — ตรวจว่าแชร์ชีทแบบ "ทุกคนที่มีลิงก์ดูได้" แล้ว');
}
...
async function load({ fresh = false } = {}) {
  ...
  const info = {
    ...
    table: `Google Sheet ${BENTO_SHEET_ID.slice(0, 8)}…${BENTO_SHEET_GID ? ` gid=${BENTO_SHEET_GID}` : ' (แท็บแรก)'}`,
