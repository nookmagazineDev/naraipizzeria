import { fetchSheet, fetchScript } from './upstream.mjs';
...
export async function callGas(url, payload, { timeoutMs = 45000 } = {}) {
  // ไม่ลองใหม่ให้เอง — payload ฝั่งนี้มีทั้งอ่านและเขียน ยิงซ้ำฝั่งเขียนแปลว่าได้แถวซ้ำในชีท
...
export async function fetchClosingRows(shape = 'lite') {
  ...
  for (const layout of CLOSING_LAYOUTS[shape]) {
    try {
      const r = await fetchSheet(layout.tq ? `${base}&tq=${encodeURIComponent(layout.tq)}` : base);
