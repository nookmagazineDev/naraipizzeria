...
async function postSchedule(base, action, payload) {
  ...
  // action ที่หน้านี้เรียกเป็นการอ่านทั้งหมด (getBranches / getHistoryData) แต่ไม่ลองใหม่ตรงนี้
  // เพราะตัวเรียกไล่ base สำรองอยู่แล้ว การลองซ้ำจะกินเวลาของ base ตัวถัดไป
  const r = await fetchUpstream(`${base}/schedule`, {
    method: 'POST',
    headers,
