...
import { dateChunks } from './dateRange';
...
/**
 * ขนาดก้อนของตารางงาน (วัน) — สาขาเดียวเดือนหนึ่ง ≈ 500 แถว ยังห่างเพดานของ office-server มาก
 * ช่วงปกติ (วันนี้/สัปดาห์นี้/เดือนนี้) จึงยังยิงครั้งเดียวเหมือนเดิม ไม่ได้เพิ่มจำนวนคำขอ
 */
export const SCHEDULE_CHUNK_DAYS = 31;

/**
 * ตารางงานของสาขาเดียวในช่วงวันที่ — คืนแถวตามรูปที่ office-server แปลงมาให้แล้ว
 *
 * ช่วงที่ยาวกว่า SCHEDULE_CHUNK_DAYS จะถูกหั่นเป็นก้อนแล้วยิงต่อกันไป ด้วยเหตุผลเดียวกับ
 * ฝั่งเวลาสแกน: เราไม่รู้เพดานแถวของ getHistoryData และถ้าชนเพดานจะมีวันหายไปเงียบๆ
 * (ยิงต่อกันทีละก้อน ไม่ยิงพร้อมกัน เพราะหลายสาขาวิ่งพร้อมกันอยู่แล้วใน fetchTimesheetMany)
 */
export async function fetchTimesheet({ branch, start, end }) {
  const out = [];
  for (const c of dateChunks(start, end, SCHEDULE_CHUNK_DAYS)) {
    const rows = await callSchedule('getHistoryData', { branch, startDate: c.start, endDate: c.end });
