...
export const MONTH_END_COLUMNS = {
  date:       ['close_date', 'month_end_date', 'end_date', 'stock_date', 'doc_date', 'period', 'month', 'date'],
  ...
  balance:    ['balance', 'end_qty', 'closing_qty', 'qty', 'quantity', 'stock_qty', 'remaining', 'remain'],
 ...
 * คืน: readMonthEndMonths / readMonthEnd
 */
export function createMonthEnd({ q }) {
  ...
  async function readLayout() {
    ...
    // เดือนของแถว: คอลัมน์ที่เป็นชนิดวันที่แปลงเป็น 'YYYY-MM' ตรง ๆ
    // ส่วนที่เก็บเป็นข้อความถือว่าเขียนแบบ ISO ('YYYY-MM-DD' หรือ 'YYYY-MM') ตัด 7 ตัวหน้าพอ
    const dateCol = bracket(found.date.name);
    const monthExpr = DATE_TYPES.has(found.date.type)
      ? `CONVERT(char(7), ${dateCol}, 126)`
      : `LEFT(CONVERT(nvarchar(20), ${dateCol}), 7)`;

    const layout = {
      columns: found,
      monthExpr,
      isDateColumn: DATE_TYPES.has(found.date.type),
  ...
  return { readMonthEndMonths, readMonthEnd, readLayout };
}
