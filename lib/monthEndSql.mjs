 ...
 *
 * ที่ร้านปิดรอบกันคาบเกี่ยวสิ้นเดือน/ต้นเดือน — ของจริงรอบสิงหาคม 2569 มีตั้งแต่ 31 ส.ค.
 * ไปจนถึง 3 ก.ย. แล้วแต่สาขา แต่ทั้งหมดนั้นคือ "รอบสิงหาคม" ชุดเดียวกัน
 * จัดกลุ่มตามเดือนของวันที่ปิดยอดตรง ๆ จะได้สองรอบทันทีที่สาขาหนึ่งปิด 31 แล้วอีกสาขาปิด 1
 * แถมสาขาที่ปิดต้นเดือนยังไปใช้ชื่อเดือนถัดไป ซึ่งไม่ตรงกับที่ออฟฟิศเรียกกัน
 *
 * กติกา: ปิดในวันที่ 1 ถึง CYCLE_PREV_MONTH_UNTIL_DAY ของเดือนไหน = รอบของ "เดือนก่อนหน้า"
 *   31 ส.ค. -> รอบ ส.ค.   |   1–10 ก.ย. -> รอบ ส.ค.   |   11 ก.ย. เป็นต้นไป -> รอบ ก.ย.
 *   1–10 มี.ค. -> รอบ ก.พ. (ไม่ต้องรู้ว่าเดือนก่อนมี 28/29/30/31 วัน)
 *
 * ตั้งไว้ 10 เผื่อสาขาที่ปิดช้ากว่าเพื่อน (ของจริงช้าสุดที่เจอคือวันที่ 3) — ไม่ได้ตั้งพอดี 3
 * เพราะถ้าเดือนไหนมีสาขาปิดวันที่ 4–5 รอบนั้นจะถูกแยกออกไปเงียบ ๆ แล้วขึ้นป้าย "ตามหลัง" ทั้งที่ปิดแล้ว
 * ส่วนการปิดรอบจริงไม่มีทางเกิดกลางเดือน เลขนี้จึงกว้างได้โดยไม่ไปดูดรอบของเดือนปัจจุบันมาผิด
 * จะเปลี่ยนเป็นเลขอื่น แก้ตรงนี้ที่เดียว (ฝั่ง SQL กับข้อความบนหน้าเว็บอ่านค่าเดียวกัน)
 */
export const CYCLE_PREV_MONTH_UNTIL_DAY = 10;
...
export function cycleMonth(isoDate) {
  ...
  // ปิดต้นเดือน = รอบของเดือนก่อน (ถอยเป็น "เดือน" ไม่ใช่ "วัน" จึงไม่ต้องสนว่าเดือนก่อนมีกี่วัน)
  const d = new Date(Date.UTC(year, month - 1 - (day <= CYCLE_PREV_MONTH_UNTIL_DAY ? 1 : 0), 1));
...
export function createMonthEnd({ q }) {
  ...
  async function readLayout() {
    ...
    // รอบเดือนของแถว: ปิดวันที่ 1–CYCLE_PREV_MONTH_UNTIL_DAY = รอบของเดือนก่อน (ดูกติกาข้างบน)
    // คอลัมน์ที่เก็บเป็นข้อความคิดแบบนี้ไม่ได้ (ไม่รู้ว่าเขียนรูปแบบไหน) จึงตัด 7 ตัวหน้าตามเดิม
    const dateCol = bracket(found.date.name);
    const isDate = DATE_TYPES.has(found.date.type);
    const monthExpr = isDate
      ? `CONVERT(char(7), DATEADD(month, CASE WHEN DAY(${dateCol}) <= ${CYCLE_PREV_MONTH_UNTIL_DAY} THEN -1 ELSE 0 END, ${dateCol}), 126)`
    ...
    const layout = {
      ...
      monthExpr,
      isDateColumn: isDate,
      // ฝั่งข้อความคิดไม่ได้ — หน้าเว็บจะได้ไม่ไปบอกผู้ใช้ว่าใช้กติกานี้อยู่ทั้งที่ไม่ได้ใช้
      cycleCutoffDay: isDate ? CYCLE_PREV_MONTH_UNTIL_DAY : 0,
