...
/**
 * ยอดที่ปิดใน "2 วันสุดท้ายของเดือน" ให้นับเป็นรอบของเดือนถัดไป
 *
 * ที่ร้านนับสต๊อกปิดรอบกันวันท้าย ๆ ของเดือน (30–31 หรือ 27–28 ของ ก.พ.) แล้วถือว่ายอดชุดนั้น
 * เป็นรอบของเดือนที่กำลังจะขึ้น ไม่ใช่เดือนที่นับ — ถ้าจัดกลุ่มตามเดือนของวันที่ปิดยอดตรง ๆ
 * รอบเดียวกันจะถูกแยกเป็นสองเดือนทันทีที่บางสาขานับ 31 และบางสาขานับ 1
 *
 * เขียนเป็น "บวก 2 วันแล้วดูว่าตกเดือนไหน" ทีเดียวจบ ไม่ต้องเช็กว่าเดือนนั้นมีกี่วัน
 *   31 ส.ค. + 2 = 2 ก.ย. -> รอบ ก.ย.   |   30 ส.ค. + 2 = 1 ก.ย. -> รอบ ก.ย.
 *   29 ส.ค. + 2 = 31 ส.ค. -> รอบ ส.ค.   |   28 ก.พ. + 2 = 2 มี.ค. -> รอบ มี.ค.
 * แก้เลขตัวเดียวตรงนี้ถ้าวันหลังเปลี่ยนกติกาเป็น 1 หรือ 3 วันสุดท้าย (ฝั่ง SQL ใช้ค่าเดียวกัน)
 */
...
export function createMonthEnd({ q }) {
  ...
  async function readLayout() {
    ...
    // รอบเดือนของแถว = เดือนของ (วันที่ปิดยอด + CYCLE_SHIFT_DAYS วัน) ตามกติกาข้างบน
