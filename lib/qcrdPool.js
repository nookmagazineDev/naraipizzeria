...
//   เครื่อง:    QCRD_DB_HOST -> ZK_DB_HOST -> HR_DB_HOST -> 203.154.185.48 (ค่าเดียวกับที่ lib/zkDb.js ใช้)
//   พอร์ต:     QCRD_DB_PORT -> ZK_DB_PORT -> HR_DB_PORT -> 14322 (พอร์ตที่เปิดออกเน็ตของอินสแตนซ์นี้)
...
function buildConfig() {
  const c = credentials();
  return {
    server: process.env.QCRD_DB_HOST || process.env.ZK_DB_HOST || process.env.HR_DB_HOST || '203.154.185.48',
    port: Number(process.env.QCRD_DB_PORT || process.env.ZK_DB_PORT || process.env.HR_DB_PORT) || 14322,
