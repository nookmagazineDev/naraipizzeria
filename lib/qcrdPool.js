...
function buildConfig() {
  ...
  return {
    ...
    // ที่ร้านเปิดไฟร์วอลล์ให้เฉพาะ IP ในไทย Vercel จึงต่อตรงไม่ติดเป็นปกติ
    //
    // ตัวจำว่า "ต่อตรงไม่ติด" (lib/directRoute.js) อยู่บน globalThis ของ instance เดียวและหมดอายุ
    // ใน 5 นาที ทุก instance ที่ Vercel เปิดใหม่จึงจ่ายค่ารอนี้ใหม่หนึ่งครั้งเสมอ ค่านี้จึงไม่ใช่
    // "เสียครั้งเดียว" แต่เป็นค่าที่คนกดบันทึกเจอเรื่อย ๆ ตอนที่ฟังก์ชันเพิ่งตื่น
    // ปลายทางที่ต่อติดใช้เวลาไม่ถึงวินาที (Vercel -> ไทย ~250 ms + handshake) 2.5 วิจึงเหลือเฟือ
    //
    // ตั้งสูงกว่านี้ด้วย QCRD_DB_CONNECT_TIMEOUT (มิลลิวินาที) ถ้าวันหลังเปิดพอร์ตให้ต่อตรงแล้ว
    // เน็ตอืดจนต่อไม่ทัน — อาการคือทุกอย่างยังทำงานได้แต่ช้าลง เพราะถอยไปใช้ tunnel ทั้งที่ไม่ต้อง
    connectionTimeout: Number(process.env.QCRD_DB_CONNECT_TIMEOUT) || 2500,
