...
export function explainDbError(err) {
  ...
  if (/is not able to access the database|Cannot open database/i.test(msg)) {
    const from = credentials().from;
    // ฐานที่ถูกปฏิเสธไม่จำเป็นต้องเป็นฐานที่ต่ออยู่ — รายชื่อพนักงานอยู่ narai_hr ซึ่งยิงข้ามฐานไป
    // เอาชื่อจากตัว error มาบอกตรง ๆ จะได้ไม่ไปให้สิทธิ์ผิดฐาน (ไฟล์ที่ต้องรันก็คนละไฟล์กัน)
    const hit = msg.match(/database "([^"]+)"/i);
    const dbName = hit ? hit[1] : (process.env.QCRD_DB_NAME || 'InventoryNarai');
    const file = /^narai_hr$/i.test(dbName) ? 'docs/schema-hr-employee.sql' : 'docs/schema-qcrd.sql';
    return `${msg}\n→ login ที่ใช้ (${from}_USER) ยังไม่มีสิทธิ์ในฐาน ${dbName} ` +
      `— ให้สิทธิ์ด้วยคำสั่งท้ายไฟล์ ${file} (CREATE USER + db_datareader/db_datawriter, ` +
      'และ db_ddladmin ถ้าจะให้สร้างตารางจากหน้าเว็บด้วย)';
  }
  if (/Invalid object name '[^']*hr_employee'/i.test(msg)) {
    return `${msg}\n→ ยังไม่มีตารางรายชื่อพนักงาน — รัน docs/schema-hr-employee.sql ที่เครื่องออฟฟิศครั้งเดียว ` +
      '(ตารางนี้อยู่ฐาน narai_hr ตัวเดียวกับตารางงาน/กะ ไม่ได้ถูกสร้างโดย step=schema)';
  }
