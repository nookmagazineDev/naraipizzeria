// ตรรกะ QC/RD บน SQL Server — ตัวที่มาแทน qcrd-apps-script.gs ทั้งไฟล์
//
// ไฟล์นี้ไม่รู้จักวิธีต่อฐานข้อมูล รับ "ตัวยิงคำสั่ง" เข้ามาแทน เพื่อให้ใช้ได้ทั้งสองทาง
// ที่หน้าเว็บมี — ต่อ SQL ตรงจาก Vercel (lib/qcrdPool.js) และผ่าน host API
// (host-server/qcrd-db.js) — โดยตรรกะการคิดต้นทุน/สูตร/cascade เป็นชุดเดียวกันเป๊ะ
// (เขียนสองที่เมื่อไหร่ = ต้นทุนที่ได้จากสองทางจะเริ่มไม่ตรงกันโดยไม่มีใครรู้)
//
//   db.q(text, params, tx)  ยิง 1 คำสั่ง คืน recordset (array)
//   db.withTx(fn)           รัน fn(tx) ใน transaction เดียว error แล้ว rollback ให้
//
// โครงตารางอยู่ใน docs/schema-qcrd.sql · ที่มาของแต่ละ action ดู docs/qcrd-sql-migration.md

// ── helpers (ต้องให้ผลตรงกับ qcrd-apps-script.gs และ lib/qcrdSheet.js) ──
export const str = v => (v === null || v === undefined ? '' : String(v).trim());
export const num = v => { const n = parseFloat(String(v ?? '').replace(/,/g, '')); return Number.isFinite(n) ? n : null; };
export const normCode = v => str(v).replace(/\.0+$/, '').replace(/^0+/, '').toLowerCase();
export const isTruthy = v => v === true || /^(y|yes|true|1|ใช่)$/i.test(str(v));
export const DEFAULT_CONVERTER = 1000;              // ตัวแปลงหน่วยเริ่มต้นเมื่อไม่ได้กรอก
// จำนวนรหัสสูงสุดที่ยอมให้ขอมาทีเดียวใน readItems(codes) — กัน IN (...) ยาวจนชน 2,100 พารามิเตอร์ของ SQL Server
export const MAX_ITEM_KEYS = 50;
const round4 = n => Math.round(n * 10000) / 10000;

/**
 * ผูกตรรกะ QC/RD เข้ากับตัวยิงคำสั่งชุดหนึ่ง
 * คืน: readMenus / readBom / readItems / readMenuGroups + actions (ชื่อเดียวกับฝั่ง Apps Script)
 */
export function createQcrd(db) {
  const { q, withTx } = db;
  ...
  /**
   * ตาราง stock_item มีคอลัมน์ use_unit แล้วหรือยัง (ถามครั้งเดียวต่อการเชื่อมต่อ)
   *
   * ⚠️ คอลัมน์นี้ถูกเพิ่มทีหลังด้วย docs/schema-qcrd.sql ซึ่งต้องรันที่เครื่องออฟฟิศเอง
   *    โค้ดขึ้นก่อนคนรันสคีมาเสมอ (Vercel deploy อัตโนมัติ) ถ้า SELECT มันไปตรง ๆ
   *    ฐานที่ยังไม่ได้รันจะตอบ "Invalid column name 'use_unit'" แล้วการอ่านทั้งหน้าล้ม
   *    → ตกไปโหมดอ่านชีท + ล็อกปุ่มบันทึกทั้งหน้า ทั้งที่ข้อมูลเดิมยังอ่านได้ทุกช่อง
   *    (อาการที่เจอจริงหลัง deploy รอบนี้) จึงถามก่อนแล้วค่อยประกอบคำสั่ง
   *
   * COL_LENGTH คืน NULL เมื่อไม่มีคอลัมน์นั้น — ไม่ต้องไปสแกน INFORMATION_SCHEMA ทั้งตาราง
   */
  let useUnitReady = null;   // null = ยังไม่ได้ถาม
  async function hasUseUnit(tx = null) {
    if (useUnitReady === null) {
      const rows = await q("SELECT COL_LENGTH('dbo.stock_item', 'use_unit') AS n", {}, tx);
      const n = rows[0]?.n;
      useUnitReady = n !== null && n !== undefined;
    }
    return useUnitReady;
  }

  /** ยังไม่มีคอลัมน์ แต่คนกรอกหน่วยใช้มา — ต้องฟ้อง ไม่ใช่เงียบแล้วทิ้งค่าที่เขาพิมพ์ */
  const needUseUnitColumn = () => new Error(
    'ยังไม่มีคอลัมน์ use_unit ในตาราง dbo.stock_item — ต้องรัน docs/schema-qcrd.sql ' +
    'ที่เครื่องออฟฟิศก่อน ถึงจะบันทึก "หน่วยใช้" ได้ (ช่องอื่นบันทึกได้ตามปกติ)'
  );

  /**
   * วัตถุดิบ (ทะเบียนเดียวกับหน้านับสต๊อก) + สาขาที่ใช้
   *
   * codes = อ่านเฉพาะรหัสที่ระบุ · ไม่ส่ง = ทั้งทะเบียน
   * มีไว้ให้หน้าเว็บตรวจ "ที่เพิ่งบันทึกเข้าจริงไหม" โดยไม่ต้องลากทะเบียนทั้งชุดกลับมา
   * (ของจริง 2,657 รายการ = JSON 1.17 MB + สแกนตารางสาขาอีกสองหมื่นกว่าแถว ทุกครั้งที่กดบันทึก)
   */
  async function readItems(codes = null) {
    const keys = Array.isArray(codes)
      ? [...new Set(codes.map(normCode).filter(Boolean))].slice(0, MAX_ITEM_KEYS)
      : null;
    if (keys && !keys.length) return [];
    // IN (...) แบบผูกพารามิเตอร์ ไม่ใช่ต่อสตริงรหัสเข้าไปในคำสั่งตรง ๆ
    const params = {};
    const where = keys
      ? ` WHERE item_key IN (${keys.map((k, i) => { params[`k${i}`] = k; return `@k${i}`; }).join(', ')})`
      : '';
    const withUseUnit = await hasUseUnit();
    const [rows, branchRows] = await Promise.all([
      q(`SELECT item_key, item_code, item_name, price, unit, status, store_cat, converter,
                ...
                ${withUseUnit ? ', use_unit' : ''}
         FROM dbo.stock_item${where}
         ORDER BY sort_order, item_code`, params),
      q(`SELECT item_key, branch FROM dbo.stock_item_branch${where}`, params),
    ]);
    const branchOf = {};
    ...
    return rows.map(r => ({
      ...
      useUnit: withUseUnit ? str(r.use_unit) : '',
    }));
  }
  ...
  async function cascadeFromMenu(srcCode, priceMap, seen, depth, tx) {
    ...
    for (const t of targets) {
      ...
      const tCost = built.rows.length ? round4(built.total) : null;
      await q('UPDATE dbo.qcrd_menu SET cost = @cost, updated_at = SYSDATETIME() WHERE menu_code = @c',
        { cost: tCost, c: tCode }, tx);
      // ส่ง cost กลับไปด้วย หน้าเว็บจะได้แก้ตัวเลขในตารางเองโดยไม่ต้องโหลดข้อมูลใหม่ทั้งชุด
      out.push({ code: tCode, name: tName, rows: built.rows.length, cost: tCost });
  ...
  const ITEM_FIELDS = [
    ...
    // หน่วยใช้ = หน่วยเล็กที่สูตรใช้จริง (คู่กับ converter ซึ่งบอกแค่ตัวเลข ไม่เคยบอกว่าหน่วยอะไร)
    ['useUnit', 'use_unit', v => str(v) || null],
  ];
  ...
  /**
   * สาขาที่อยู่ในฐาน "จริง ๆ" ของวัตถุดิบตัวหนึ่ง (ตัวพิมพ์ใหญ่ กติกาเดียวกับ readItems)
   *
   * อ่านกลับมาหลังเขียนเสร็จ แล้วคืนไปกับผลการบันทึก เพื่อให้หน้าเว็บเอาไปแสดงได้ทันที
   * โดยไม่ต้องเดาจากค่าที่กรอก — และถ้าวันไหนเขียนไม่เข้าจริง ค่าที่คืนไปจะไม่ตรงกับที่ส่งมา
   * ซึ่งดีกว่าขึ้น "สำเร็จ" เฉย ๆ แล้วปล่อยให้คนใช้ไปเจอเองว่าข้อมูลไม่เปลี่ยน
   */
  async function readItemBranches(itemKey, tx) {
    const rows = await q(
      'SELECT branch FROM dbo.stock_item_branch WHERE item_key = @k ORDER BY branch', { k: itemKey }, tx);
    return rows.map(r => str(r.branch).toUpperCase());
  }
  ...
  async function saveItem(data) {
    ...
    return withTx(async (tx) => {
      ...
      const params = { k: key };
      const withUseUnit = await hasUseUnit(tx);
      if (!withUseUnit && str(data.useUnit)) throw needUseUnitColumn();
      ITEM_FIELDS.forEach(([field, col, conv], i) => {
        if (data[field] === undefined) return;
        if (field === 'useUnit' && !withUseUnit) return;         // ฐานยังไม่มีคอลัมน์ (ค่าว่างเท่านั้นถึงมาถึงตรงนี้)
        if (field === 'name' && !str(data.name)) return;         // ชื่อว่าง = ไม่ทับของเดิม
        // ราคาว่าง = ล้างค่า (เดิมข้ามไปเฉย ๆ คนลบราคาจึงเห็นราคาเก่าค้างทั้งที่ขึ้นว่าบันทึกแล้ว)
        // ส่วนค่าที่ไม่ใช่ตัวเลขถือว่าพิมพ์ผิด ไม่เขียนทับของเดิม — ตรงกับ saveItem_ ฝั่ง Apps Script
      ...
      return { code, key, branches: await readItemBranches(key, tx) };
    });
  }
      ...
      await q(`
        INSERT INTO dbo.stock_item
          (item_key, item_code, item_name, unit, price, status, store_cat, converter,
           sub_item1, sub_item2, sub_item3, item_type, used_when, pos_item_id, request_unit,
           ${withUseUnit ? 'use_unit,' : ''} sort_order)
        VALUES (@k, @code, @name, @unit, @price, @status, @store, @conv,
                @s1, @s2, @s3, @type, @used, @pos, @runit,
                ${withUseUnit ? '@uunit,' : ''} @sort)`, {
        k: key, code, name,
        ...
        ...(withUseUnit ? { uunit: str(data.useUnit) || null } : {}),
        sort: (Number(maxSort) || 0) + 1,
      }, tx);
      if (data.branches !== undefined) await writeItemBranches(key, data.branches, tx);
      return { code, key, branches: await readItemBranches(key, tx) };
  ...
  /**
   * คัดลอก "สาขาที่ใช้" จากสาขาต้นแบบไปอีกสาขา — payload { from, to, mode }
   *
   * เปิดสาขาใหม่แล้วต้องไล่ติ๊กสาขาทีละวัตถุดิบเป็นพันตัว ทำทีเดียวจบตรงนี้แทน
   *   mode 'add'     = เติมสาขาปลายทางให้ทุกตัวที่สาขาต้นแบบใช้ (ของที่ปลายทางมีอยู่แล้วคงไว้)
   *   mode 'replace' = ปลายทางเหลือเฉพาะชุดเดียวกับต้นแบบเป๊ะ (ตัวที่ต้นแบบไม่ใช้ เอาปลายทางออก)
   * แตะเฉพาะแถวของสาขาปลายทาง สาขาอื่นของวัตถุดิบแต่ละตัวไม่เปลี่ยน
   */
  async function copyBranchItems(data) {
    const from = str(data.from).toLowerCase();
    const to = str(data.to).toLowerCase();
    const mode = str(data.mode) === 'replace' ? 'replace' : 'add';
    if (!from || !to) throw new Error('ต้องระบุสาขาต้นแบบและสาขาปลายทาง');
    if (from === to) throw new Error('สาขาต้นแบบกับสาขาปลายทางต้องไม่ใช่สาขาเดียวกัน');
    return withTx(async (tx) => {
      const src = Number((await q(
        'SELECT COUNT(*) AS n FROM dbo.stock_item_branch WHERE LOWER(branch) = @f', { f: from }, tx))[0]?.n) || 0;
      // ต้นแบบว่าง + โหมดแทนที่ = ล้างปลายทางทิ้งทั้งหมด เกือบแน่นอนว่าเลือกสาขาผิด
      if (!src) throw new Error(`สาขา ${from.toUpperCase()} ยังไม่มีวัตถุดิบให้คัดลอก`);
      let removed = 0;
      if (mode === 'replace') {
        const r = await q(`
          DELETE t FROM dbo.stock_item_branch t
          WHERE LOWER(t.branch) = @t
            AND NOT EXISTS (SELECT 1 FROM dbo.stock_item_branch s
                            WHERE s.item_key = t.item_key AND LOWER(s.branch) = @f);
          SELECT @@ROWCOUNT AS n`, { f: from, t: to }, tx);
        removed = Number(r[0]?.n) || 0;
      }
      const a = await q(`
        INSERT INTO dbo.stock_item_branch (item_key, branch)
        SELECT DISTINCT s.item_key, @t FROM dbo.stock_item_branch s
        WHERE LOWER(s.branch) = @f
          AND NOT EXISTS (SELECT 1 FROM dbo.stock_item_branch t
                          WHERE t.item_key = s.item_key AND LOWER(t.branch) = @t);
        SELECT @@ROWCOUNT AS n`, { f: from, t: to }, tx);
      const added = Number(a[0]?.n) || 0;
      const total = Number((await q(
        'SELECT COUNT(*) AS n FROM dbo.stock_item_branch WHERE LOWER(branch) = @t', { t: to }, tx))[0]?.n) || 0;
      return { from: from.toUpperCase(), to: to.toUpperCase(), mode, source: src, added, removed, total };
    });
  }
  ...
  const ACTIONS = {
    ...
    saveItem, addItem, deleteItem, updateItemUnits, copyBranchItems, sortBom,
  };

  return { readMenus, readBom, readItems, readMenuGroups, actions: ACTIONS };
}
