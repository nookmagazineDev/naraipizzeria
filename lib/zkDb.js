...
//
//   1) office-server /attendance ที่ storenarai.dyndns.tv:8787  ← ทางหลัก
//      เซิร์ฟเวอร์ตัวเดียวกับที่ /api/usagemenu, /api/usagebytable ใช้อยู่แล้ว
//      และเป็นทางที่โปรเจค Narai-branch ใช้ดึงหน้า "สแกนเข้า-ออก" มาตลอด
...
// Narai Usage API ที่รันในออฟฟิศ — ตัวเดียวกับที่ /api/usagemenu, /api/usagebytable ใช้อยู่
// และเป็นตัวที่โปรเจค Narai-branch ใช้ดึงหน้า "สแกนเข้า-ออก" มาตลอด (ผ่าน /attendance)
// (ไม่ใช่ข้อมูลลับ — เป็น dyndns สาธารณะ)
export const ZK_OFFICE_API_BASE =
  process.env.ZK_OFFICE_API_BASE || process.env.USAGE_API_BASE || 'http://storenarai.dyndns.tv:8787';
...
export async function fetchAttendanceViaOffice({ start, end, branch, emp }) {
  ...
  const r = await fetch(`${ZK_OFFICE_API_BASE}/attendance?${p.toString()}`, {
    cache: 'no-store',
    signal: AbortSignal.timeout(HTTP_TIMEOUT_MS),
  });
  const j = await r.json().catch(() => null);
  if (!r.ok || !j || j.status !== 'success') {
    const e = new Error((j && j.message) || `office-server ตอบ HTTP ${r.status}`);
    e.status = r.status;
    throw e;
  }
  return j.data || [];
