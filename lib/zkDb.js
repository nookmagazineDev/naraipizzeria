...
g.__zkOfficeBaseOk = g.__zkOfficeBaseOk || null; // base ของ office-server ที่ใช้ได้ล่าสุด
...
async function officeGetAttendance(base, qs) {
  const r = await fetch(`${base}/attendance?${qs}`, {
    cache: 'no-store',
    signal: AbortSignal.timeout(OFFICE_TRY_MS),
  });
 ...
 * จึงคืนรูปที่หน้าเว็บใช้ได้เลย
 *
 * ฝั่งนั้นมีเพดานแถวของมันเอง และเราไม่รู้ว่ามันตัดหัวหรือตัดท้าย (เคยเจอว่าขอเป็นเดือน
 * แล้ววันท้ายๆ หายไปทั้งวัน) /api/attendance จึงขอเป็นก้อนละไม่กี่วันแทนการขอทีเดียวทั้งเดือน
 * ที่นี่ทำหน้าที่แค่ยิงตามช่วงที่ขอมา
 *
 * base ที่สำเร็จจะถูกจำไว้แล้วลองก่อนในครั้งถัดไป (เหมือน lib/hrSchedule.js) — สำคัญเมื่อ
 * ต้องยิงหลายก้อนติดกัน เพราะ base ที่เข้าไม่ได้จะค้างจน timeout ทุกก้อน
 */
export async function fetchAttendanceViaOffice({ start, end, branch, emp }) {
  ...
  const bases = g.__zkOfficeBaseOk
    ? [g.__zkOfficeBaseOk, ...ZK_OFFICE_API_BASES.filter((b) => b !== g.__zkOfficeBaseOk)]
    : [...ZK_OFFICE_API_BASES];

  const fails = [];
  for (const base of bases) {
    try {
      const data = await officeGetAttendance(base, qs);
      g.__zkOfficeBaseOk = base;
      return data;
    } catch (e) {
      fails.push(`${base} → ${e.message}`);
    }
  }
  g.__zkOfficeBaseOk = null;
...
export async function fetchPunchesViaHost({ start, end, branch, emp }) {
  ...
  return rows
    ...
    .sort((a, b) => b.time.localeCompare(a.time)); // หน้าเว็บอยากได้ใหม่ก่อน (host บิลด์เก่าเรียงเก่า→ใหม่)
}
