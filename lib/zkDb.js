...
//   ZK_SOURCE       ตั้งเป็น sql = สลับให้ต่อ SQL ตรงเป็นทางหลัก (host API กลายเป็นทางสำรอง)
...
/**
 * ZK_SOURCE=sql (หรือ sql-direct) = ให้ต่อ SQL ตรงเป็น "ทางหลัก" แทน host API
 * ใช้เมื่อเลือกเปิดพอร์ต SQL ออกเน็ตแล้วไม่อยากพึ่ง host-server บนเครื่องออฟฟิศ
 * (host API ยังเป็นทางสำรองให้อยู่ ไม่ได้ตัดทิ้ง)
 */
export const preferDirectDb = () => /^sql/i.test(process.env.ZK_SOURCE || '');
