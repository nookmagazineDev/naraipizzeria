...
export const SHEETS_API_BASE = (
  process.env.SHEETS_API_BASE || process.env.QCRD_API_BASE || STORE_API_BASE
).replace(/\/+$/, '');
...
async function getFromHost(path, { timeoutMs = 20000 } = {}) {
  ...
  const res = await fetchUpstream(`${SHEETS_API_BASE}/sheets/${path}`, { timeoutMs });
...
async function postToHost(body, { timeoutMs = 60000 } = {}) {
  ...
  const res = await fetchUpstream(`${SHEETS_API_BASE}/sheets/save`, {
    ...WRITE_UPSTREAM_OPTS,
    method: 'POST',
...
// พนักงานเรียกตัวนี้เสมอ ไม่ผ่านสวิตช์ SHEETS_SOURCE และไม่มีทางถอยไปชีท (ดู pages/api/stock-gas.js)
// รายชื่อพนักงานย้ายไปฐาน narai_hr แล้ว (ตารางเดียวกับที่ Narai-branch ใช้ลงตารางงาน)
// อ่าน/เขียนที่ lib/hrEmployee.mjs — อย่าเพิ่มทางกลับมาที่นี่อีก ไม่งั้นจะกลายเป็นสองชุดเหมือนเดิม
