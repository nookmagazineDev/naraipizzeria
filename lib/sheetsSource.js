...
// ไม่มีทางถอยไปชีท (ข้อมูลชุดนี้ไม่เคยอยู่ในชีท) ต่อฐานไม่ได้ = หน้านั้นขึ้นข้อความว่าอ่านไม่ได้

/** แถวปิดรอบของเดือนหนึ่ง (ไม่ระบุเดือน = เดือนล่าสุดที่มีข้อมูล) */
export const readMonthEnd = ({ month, branch } = {}) => {
  if (hasDirectDb()) return directMonthEnd.readMonthEnd({ month, branch });
  const p = new URLSearchParams();
  if (month) p.set('month', month);
  if (branch) p.set('branch', branch);
  const qs = p.toString();
  return getFromHost(`month-end${qs ? `?${qs}` : ''}`);
};

/** เดือนที่มีข้อมูลปิดรอบ ('YYYY-MM' ใหม่ก่อน) */
export const readMonthEndMonths = () =>
  (hasDirectDb() ? directMonthEnd.readMonthEndMonths() : getFromHost('month-end-months'));
