...
import { createBranchUsers } from './branchUserSql.mjs';
import { createStockCount } from './stockCountSql.mjs';
import { createUniform } from './uniformSql.mjs';
...
/* ผู้ใช้ฝั่งสาขา (narai_hr.dbo.hr_user) — คนละฐานแต่อินสแตนซ์เดียวกัน อ่าน/เขียนด้วยชื่อสามท่อน
   ต้องให้สิทธิ์ข้ามฐานก่อน (docs/grant-hr-user.sql) ไม่งั้นจะมองไม่เห็นตารางทั้งที่มีอยู่จริง */
const directBranchUsers = createBranchUsers({ q: runQuery });
...
/** ยูนิฟอร์มของสาขา (dbo.UniformBranch) — ฐานเดียวกัน ดูอย่างเดียว ดู lib/uniformSql.mjs */
const directUniform = createUniform({ q: runQuery });
...
// "ต่อ SQL Server (inventory.dyndns.tv:1433/InventoryNarai) ไม่ได้: Failed to connect ... in 2500ms"
...
// (ฟังก์ชันตั้ง maxDuration ไว้ 60 วิ ต่อตรงก่อนหน้าใช้ไปไม่เกิน 2.5 วิ ดู connectionTimeout ใน qcrdPool)
...
function readCalendarMonth(month, branch, limit) {
  const p = new URLSearchParams();
  if (month) p.set('month', month);
  if (branch) p.set('branch', branch);
  // host เวอร์ชันเก่าไม่รู้จัก limit ก็แค่ใช้เพดานเดิมของตัวเอง ไม่ได้พัง
  if (limit) p.set('limit', String(limit));
  const qs = p.toString();
  return monthEndRead(
    () => directMonthEnd.readMonthEnd({ month, branch, limit }),
    `month-end${qs ? `?${qs}` : ''}`);
}
 ...
 */
export async function readMonthEnd({ month, branch, limit } = {}) {
  const first = await readCalendarMonth(month, branch, limit);
  ...
  const fetched = await Promise.all(need.map(async (m) => {
    ...
    try { return await readCalendarMonth(m, branch, limit); }
...
/* ---- รหัสพ้อง และเป้ายอด/เพดานค่าแรง (เพิ่มตอนยกทะเบียนนี้เป็นทะเบียนแม่) ----
   ฐานที่ยังไม่ได้รัน docs/schema-hr-branch.sql รอบใหม่จะยังไม่มีสองตารางนี้
   ฝั่งอ่านคืนลิสต์ว่างให้เอง (ดู createBranches ใน lib/branchSql.mjs) ไม่ใช่ error
   ฝั่งเขียนตอบกลับว่าให้ไปรันสคีมาก่อน — จะได้ไม่ต้องเดาว่าทำไมกดแล้วเงียบ     */

export const readBranchAliases = () =>
  pickRoute('อ่านรหัสพ้องสาขา', () => directBranches.readAliases(), () => getFromHost('branch-alias'));

export const saveAliasRow = (body) =>
  pickRoute('บันทึกรหัสพ้อง',
    () => directBranches.actions.saveAlias(body),
    () => postToHost({ ...body, action: 'saveAlias' }));

export const deleteAliasRow = (body) =>
  pickRoute('ลบรหัสพ้อง',
    () => directBranches.actions.deleteAlias(body),
    () => postToHost({ ...body, action: 'deleteAlias' }));

export const saveTargetRow = (body) =>
  pickRoute('บันทึกเป้ายอดสาขา',
    () => directBranches.actions.saveTarget(body),
    () => postToHost({ ...body, action: 'saveTarget' }));

/* ---- ยูนิฟอร์มของสาขา (dbo.UniformBranch) — หน้า HR → ยูนิฟอร์ม ดูอย่างเดียว ----
   ไม่มีชีทให้ถอยกลับ — ข้อมูลชุดนี้อยู่ในฐานอย่างเดียว ต่อไม่ได้ทั้งสองทาง = ตอบ error
   คืนทางที่ใช้จริงมาด้วย (source) หน้าเว็บเอาไปขึ้นบอกว่าอ่านมาจากไหน */
export async function readUniformBranch({ limit } = {}) {
  const { data, route } = await viaDirectOrHost('อ่านยูนิฟอร์มของสาขา',
    () => directUniform.readUniformBranch({ limit }),
    () => getFromHost(`uniform-branch${limit ? `?limit=${encodeURIComponent(limit)}` : ''}`,
      { timeoutMs: 45000 }));
  return { ...data, source: route };
}

/** ใบขอเบิกยูนิฟอร์มของสาขา (dbo.stock_request คัดเฉพาะไอเทมยูนิฟอร์ม) — หน้า HR → ยูนิฟอร์ม */
export async function readUniformRequests({ limit } = {}) {
  const { data, route } = await viaDirectOrHost('อ่านใบขอเบิกยูนิฟอร์ม',
    () => directUniform.readUniformRequests({ limit }),
    () => getFromHost(`uniform-request${limit ? `?limit=${encodeURIComponent(limit)}` : ''}`,
      { timeoutMs: 45000 }));
  return { ...data, source: route };
}

/** ตั้งสถานะใบขอเบิกยูนิฟอร์ม (อนุมัติแล้ว · รอสั่งสินค้า / กำลังจัดส่ง) — dbo.uniform_request_status */
export const setUniformRequestStatusRow = (body) =>
  pickRoute('ตั้งสถานะใบขอเบิกยูนิฟอร์ม',
    () => directUniform.actions.setUniformRequestStatus(body),
    () => postToHost({ ...body, action: 'setUniformRequestStatus' }));

/* ---- login ของสาขา (narai_hr.dbo.hr_user) ----
   บัญชีที่สาขาใช้เข้าระบบลงตารางงาน เดิมไม่มีหน้าจอ ต้องเปิด SSMS แก้เอง
   ⚠️ ทางที่ใช้ได้จริงคือ host API เท่านั้น — ฐาน narai_hr อยู่บนเครื่องออฟฟิศ
      ต่อ SQL ตรงจาก Vercel จะไปไม่ถึง (และถ้าไปถึงก็เป็นคนละอินสแตนซ์)
      ตัวตรวจใน lib/branchUserSql.mjs จะบอกเองว่าให้ไปทางไหน                 */

export const readBranchUsers = () =>
  pickRoute('อ่าน login ของสาขา',
    () => directBranchUsers.readBranchUsers(), () => getFromHost('branch-user'));

export const saveBranchUserRow = (body) =>
  pickRoute('บันทึก login ของสาขา',
    () => directBranchUsers.actions.saveBranchUser(body),
    () => postToHost({ ...body, action: 'saveBranchUser' }));

export const setBranchUserPasswordRow = (body) =>
  pickRoute('ตั้งรหัสผ่านของสาขา',
    () => directBranchUsers.actions.setBranchUserPassword(body),
    () => postToHost({ ...body, action: 'setBranchUserPassword' }));

export const deleteBranchUserRow = (body) =>
  pickRoute('ลบ login ของสาขา',
    () => directBranchUsers.actions.deleteBranchUser(body),
    () => postToHost({ ...body, action: 'deleteBranchUser' }));
