...
// (Vercel ใช้ instance ซ้ำระหว่าง request ตัวแปรนี้จึงอยู่ข้ามคำขอได้ — instance ใหม่ก็แค่ลองต่อตรงอีกครั้ง
//  ซึ่งเป็นสิ่งที่ต้องการอยู่แล้ว วันไหนเปิดพอร์ตจริงจะได้กลับไปใช้ทางเร็วเองโดยไม่ต้อง deploy ใหม่)
...
async function monthEndRead(runDirect, hostPath) {
  ...
  if (Date.now() < directDownUntil) return fromHost('ต่อ SQL ตรงไม่ติดเมื่อครู่ จึงข้ามมาทางนี้เลย');
  ...
  try {
    ...
    directDownUntil = Date.now() + DIRECT_COOLDOWN_MS;
