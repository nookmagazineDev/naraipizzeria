...
async function monthEndRead(runDirect, hostPath) {
  ...
  if (!hasDirectDb()) return { data: await getFromHost(hostPath), route: hostRoute };

  try {
    return { data: await runDirect(), route: `ต่อ SQL ตรง (${describeTarget()})` };
  } catch (err) {
    ...
    try {
      return { data: await getFromHost(hostPath), route: `${hostRoute} — ต่อ SQL ตรงไม่ได้จึงถอยมาทางนี้` };
    } catch (hostErr) {
