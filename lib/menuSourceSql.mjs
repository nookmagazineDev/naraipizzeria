...
export function menuSources() {
  return [
    ...
    },
    {
      ...
      // ตารางนี้ไม่มีคอลัมน์รหัสเมนูจริง ๆ มีแต่ id ที่เป็น epoch millisecond 13 หลัก
      // (1785999718411) ซึ่งเอามาเป็นรหัสเมนูแล้วยาวเกินกว่าจะใช้งานไหว — หน้าเว็บจึงออก
      // เลขรันนิ่ง 6 หลักให้แทน (HM000001, HM000002, …) นับต่อจากรหัส HM ที่มีในทะเบียนแล้ว
      // ⚠️ ไม่ใช้คอลัมน์ Seq ของต้นทางเป็นรหัส เพราะมันคือ "ลำดับ" ที่เรียงใหม่/เลื่อนได้
      //    วันหลังมีคนจัดเรียงเมนู รหัสเดิมจะไปตรงกับเมนูคนละตัวโดยไม่มีอะไรฟ้อง
      codeMode: 'running6',
    },
    {
      id: 'naraipos',
...
export function createMenuSource({ q }) {
  ...
  async function schema({ fresh = false } = {}) {
    // ⚠️ ต้องเช็กว่า "ต่อฐานได้ไหม" แยกจากการไล่ทีละต้นทางก่อนเสมอ
    //
    // ข้างล่างจับ error ของแต่ละต้นทางไว้เองเพื่อไม่ให้ตัวเดียวล้มทั้งชุด — ผลข้างเคียงคือ
    // ตอน "ต่อฐานไม่ติดเลย" (ฝั่ง Vercel ต่อ SQL ตรงไม่ได้เป็นปกติ) ทั้งสามต้นทางจะกลายเป็น
    // ok:false ทั้งแถว แล้ว "สำเร็จ" กลับไป ผู้เรียกจึงไม่รู้ว่าต้องถอยไปทาง host API
    // (อาการที่เจอจริง: ตารางมีข้อมูลเพราะ search ถอยไป host ได้ แต่แท็บขึ้น "อ่านไม่ได้" ทั้งสามอัน
    //  พร้อม error ว่าต่อ inventory.dyndns.tv:1433 ไม่ได้)
    // คำสั่งเบา ๆ ตัวนี้ล้มเมื่อไหร่ = ไปไม่ถึงฐาน ปล่อยให้ error หลุดออกไปให้ viaDirectOrHost ถอยทางเอง
    ...
    return Promise.all(menuSources().map(async (src) => {
      try {
        ...
        return {
          id: s.id, label: s.label, prefix: s.prefix, codeMode: s.codeMode || 'source',
          table: `${s.db}.dbo.${s.table}`,
          ok: true, rows: Number(count[0]?.n) || 0, map: s.map, columns: s.columns,
        };
      } catch (err) {
        return {
          id: src.id, label: src.label, prefix: src.prefix, codeMode: src.codeMode || 'source',
          table: `${src.db}.dbo.${src.table}`,
  ...
  async function search(srcId, { q: keyword = '', limit = 50, includeInactive = false } = {}) {
    ...
    const mapped = rows.map(r => {
      ...
      return {
        ...
        // ต้นทางแบบ running6 ให้หน้าเว็บเป็นคนออกเลข (มันรู้ว่าทะเบียนมีรหัสอะไรอยู่แล้วบ้าง)
        // ส่งค่านี้มาด้วยไว้เป็นทางถอยเฉย ๆ ไม่ได้ใช้บันทึกจริง
    ...
    return {
      source: {
        id: s.id, label: s.label, prefix: s.prefix, codeMode: s.codeMode || 'source',
        table: `${s.db}.dbo.${s.table}`, map: s.map,
      },
      rows: kept,
