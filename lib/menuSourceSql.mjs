...
const FIELDS = {
  ...
  group: ['CategoryName', 'GroupName', 'MenuGroupName', 'ItemGroupName',
    'CategoryCode', 'GroupCode', 'MenuGroup', 'ItemGroup', 'ItemGrp', 'Category', 'CategoryId'],
...
export function createMenuSource({ q }) {
  ...
  async function detect(src, { fresh = false } = {}) {
    ...
    const map = {};
    for (const [field, cands] of Object.entries(FIELDS)) map[field] = pickColumn(columns, cands);
