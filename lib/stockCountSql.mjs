...
export function createStockCount({ q }) {
  ...
  async function readStockItems(branch) {
    ...
    const [items, counts, balances, requests, categories, calcSettings] = await Promise.all([
      ...
      q(`SELECT item_key, item_code, avg_qty, par_qty, calc_mode FROM dbo.stock_avg_per_head
          WHERE branch IN (@avgAlias0, @avgAlias1)`, { avgAlias0, avgAlias1 }).catch((err) => {
        ...
        console.error(`stockCountSql: อ่านค่าตั้งเบิกของสาขา ${branchKey} ไม่ได้ — คอลัมน์จะว่าง:`, err.message);
        return [];
      }),
    ]);
    ...
    // จับคู่ด้วย item_key เป็นหลัก (ฝั่งสาขาใช้ normCode(รหัสสินค้า) เหมือนกันทั้งสองตาราง)
    // แต่แถวเก่าที่ย้ายมาจากชีทเคยมี key ไม่ได้ normalize — ลงรหัสสินค้าที่ normalize แล้วไว้อีกทาง
    // จะได้ไม่เงียบหายเพราะ key เพี้ยนตัวเดียว (ไม่ทับของที่จับคู่ด้วย item_key ได้อยู่แล้ว)
    const calcByItem = new Map();
    calcSettings.forEach(c => {
      const alt = normKey(c.item_code);
      if (alt && !calcByItem.has(alt)) calcByItem.set(alt, c);
    });
    calcSettings.forEach(c => calcByItem.set(c.item_key, c));
