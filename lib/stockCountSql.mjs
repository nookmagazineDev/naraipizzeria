...
/** รหัสสินค้า -> คีย์ (ตัด .0 ท้าย + 0 นำหน้า) — ตรงกับ normCode ฝั่งสาขาที่ใช้สร้าง item_key */
const normKey = (v) => str(v).replace(/\.0+$/, '').replace(/^0+/, '').toLowerCase();
...
export function createStockCount({ q }) {
  ...
  async function readStockItems(branch) {
    ...
    const [items, counts, balances, requests, categories, calcSettings] = await Promise.all([
      ...
      //
      // ดึง "ทุกสาขา" ไม่ใช่เฉพาะสาขาที่เปิดดู เพราะหน้าเว็บให้กดที่ตัวเลขแล้วเทียบกับสาขาอื่นได้
      // เอาเฉพาะแถวที่ตั้งค่าไว้จริง (ตารางเก็บแถวที่ avg_qty = 0 ไว้ด้วยตอนติ๊กโหมด par เปล่า ๆ)
      // จึงไม่ได้ใหญ่ตามจำนวนสินค้า x สาขา — คัดที่ฝั่งฐานถูกกว่าขนมาทั้งตารางแล้วมากรองที่นี่
      //
      ...
      q(`SELECT branch, item_key, item_code, avg_qty, par_qty, calc_mode
           FROM dbo.stock_avg_per_head
          WHERE avg_qty > 0 OR par_qty IS NOT NULL`).catch((err) => {
        // เงียบไม่ได้ — คอลัมน์ว่างทั้งตารางเพราะอ่านไม่ได้ กับเพราะสาขายังไม่ได้ตั้งค่า
        // หน้าตาเหมือนกันเป๊ะ ถ้าไม่บอกไว้ตรงนี้จะไล่หาสาเหตุไม่เจอ
        console.error('stockCountSql: อ่านค่าตั้งเบิกไม่ได้ — คอลัมน์จะว่าง:', err.message);
        return [];
      }),
    ]);
    ...
    // แถวค่าตั้งเบิกจับคู่กับสินค้าด้วย item_key เป็นหลัก (ฝั่งสาขาใช้ normCode(รหัสสินค้า)
    // เหมือนกันทั้งสองตาราง) แต่แถวเก่าที่ย้ายมาจากชีทเคยมี key ไม่ได้ normalize
    // จึงลงรหัสสินค้าที่ normalize แล้วไว้อีกทาง จะได้ไม่เงียบหายเพราะ key เพี้ยนตัวเดียว
    const calcKeysOf = (c) => {
      const alt = normKey(c.item_code);
      return alt && alt !== c.item_key ? [c.item_key, alt] : [c.item_key];
    };
    const aliasSet = new Set([avgAlias0, avgAlias1]);
    const calcByItem = new Map();          // ค่าของสาขาที่เปิดดูอยู่
    const calcBranchesByItem = new Map();  // ทุกสาขา — ไว้ให้กดดูเทียบกัน
    calcSettings.forEach(c => {
      const entry = {
        branch: str(c.branch),
        calcMode: calcModeOf(c.calc_mode),
        avgPerHead: Number(c.avg_qty) > 0 ? Number(c.avg_qty) : '',
        parQty: c.par_qty === null || c.par_qty === undefined ? '' : Number(c.par_qty),
      };
      for (const k of calcKeysOf(c)) {
        const list = calcBranchesByItem.get(k) || [];
        list.push(entry);
        calcBranchesByItem.set(k, list);
        // สาขาที่เปิดดูอยู่ชนะเสมอ ไม่ว่าจะเจอแถวไหนก่อน (แถวของ alias อีกชื่อก็นับ)
        if (aliasSet.has(entry.branch)) calcByItem.set(k, c);
      }
    });
    calcBranchesByItem.forEach(list => list.sort((a, b) => a.branch.localeCompare(b.branch)));
    ...
    return items.map(it => {
      ...
      const calc = calcByItem.get(it.item_key) || calcByItem.get(normKey(it.item_code)) || null;
      ...
      return {
        ...
        // ค่าของทุกสาขาที่ตั้งไว้ — หน้าเว็บกดที่ตัวเลขแล้วเปิดตารางเทียบ
        calcBranches: calcBranchesByItem.get(it.item_key)
          || calcBranchesByItem.get(normKey(it.item_code)) || [],
      };
    });
  }
