...
export function mapItems(rows) {
  ...
  rows.forEach((r, index) => {
    ...
    byKey.set(key, {           // รหัสซ้ำ = แถวท้ายสุดของชีทชนะ (ตรงกับที่ชีทใช้ค่าล่าสุด)
      ...
      useUnit: str(r[16]) || null,     // Q หน่วยใช้ (หน่วยเล็กในสูตร)
      sortOrder: index,
    });
  });
  ...
  return {
    text: `
      ...
      WHEN MATCHED THEN UPDATE SET
        ...
        use_unit = COALESCE(${p('use_unit')}, t.use_unit),
      ...
      WHEN NOT MATCHED THEN INSERT
        (item_key, item_code, item_name, unit, price, status, store_cat, converter,
         sub_item1, sub_item2, sub_item3, item_type, used_when, use_unit,
        ...
        VALUES (${p('item_key')}, ${p('item_code')}, ${p('item_name')}, ${p('unit')}, ${p('price')},
                ...
                ${p('sub1')}, ${p('sub2')}, ${p('sub3')}, ${p('item_type')}, ${p('used_when')}, ${p('use_unit')},
