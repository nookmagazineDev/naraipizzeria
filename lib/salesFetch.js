...
// ค่าที่เก่ากว่านี้ไม่ใช่ "วันเปิดบิล" จริง แต่เป็นค่าหลอกที่ติดมากับชนิดข้อมูล:
//   - คอลัมน์ชนิด time ของ SQL Server → driver คืนเป็น Date ที่ปักวันไว้ 1970-01-01
//   - ค่าฐานของ datetime รุ่นเก่า → 1900-01-01
// ทั้งสองแบบ "ดูเหมือนวันที่" จนผ่านการเช็กรูปแบบ แต่ใช้เทียบช่วงวันไม่ได้เลย
const MIN_REAL_DATE = '2000-01-01';

/** อ่านเป็น 'YYYY-MM-DD' ถ้าค่านั้นมีวันที่จริงอยู่ — ไม่งั้นคืนค่าว่างเพื่อให้ไปใช้ทางถอย */
const realYmd = v => {
  const s = String(v ?? '').trim();
  if (!/^\d{4}-\d{2}-\d{2}/.test(s)) return '';   // เวลาล้วน '18:30:00' · แบบไทย '17/09/2026'
  const ymd = s.slice(0, 10);
  return ymd >= MIN_REAL_DATE ? ymd : '';
};

/**
 * ยึด "วันที่เปิดบิล" (startTime) เป็นหลักทุกเมนู; ไม่ได้ก็ถอยไปใช้วันที่อื่นของแถวนั้น
 *
 * ⚠️ ทางถอยต้องครอบคลุมสองเรื่องที่เคยพลาดมาแล้วทั้งคู่:
 *
 * 1) startTime "อ่านเป็นวันที่ไม่ได้" ไม่ใช่แค่ "ว่าง" — ของเดิมเช็กแค่ `if (t)` แล้ว
 *    slice(0,10) ดื้อ ๆ ถ้าคอลัมน์ StartTime เป็นชนิด time หรือเก็บเวลาล้วน จะได้
 *    '1970-01-01'/'18:30:00' ออกมาเป็น "วันของแถว" ซึ่งไม่มีวันตรงกับช่วงที่ผู้ใช้เลือก
 *
 * 2) แถว "รายการ" (Ctrans) ไม่มีคอลัมน์ date/Date เลย — มีแต่ postTime (คอลัมน์ที่
 *    /ctranbetweendate ใช้กรองช่วงวันอยู่แล้ว) ถ้าไม่ถอยมาใช้ postTime แถวรายการจะได้
 *    วันเป็น '-' แล้วหลุดตัวกรองช่วงวันทั้งหมด ส่วนแถว "บิล" (Cpaid) ที่มี date ยังรอด
 *    → หน้ายอดรายวันขึ้นยอดขาย/จำนวนบิลปกติ แต่ทุกคอลัมน์ที่คิดจากรายการกลายเป็น 0
 *    (จำนวนหัว · จานบุฟเฟต์ · เด็กฟรี · ผู้สูงอายุฟรี · ต้นทุน · %ต้นทุน) โดยไม่มี error
 */
export const dateFromRow = row =>
  realYmd(row['startTime'])
  || realYmd(row['Date'] || row['date'])
  || realYmd(row['postTime'] || row['PostTime'])
  || '-';
