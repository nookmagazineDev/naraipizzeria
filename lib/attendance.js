...
/**
 * ชั่วโมงแบบทศนิยม -> 'ช:นน' (1 ชม. = 60 นาที) · 9.0667 -> '9:04' · null -> ''
 *
 * ตารางเวลาทำงานอยู่บรรทัดเดียวกับเวลาสแกนที่เป็นนาฬิกา ตัวเลขทศนิยมจึงอ่านผิดกันประจำ
 * — "พัก 1.11" ไม่ใช่ 1 ชม. 11 นาที แต่เป็น 1 ชม. 7 นาที
 * ปัดเป็นนาทีทั้งก้อนก่อนค่อยหารเป็นชั่วโมง ไม่งั้น 1.999 ชม. จะกลายเป็น '1:60'
 */
export function hoursToHm(h) {
  if (h == null || !Number.isFinite(h)) return '';
  const mins = Math.round(Math.abs(h) * 60);
  return `${h < 0 ? '-' : ''}${Math.floor(mins / 60)}:${String(mins % 60).padStart(2, '0')}`;
}
...
/**
 * หมายเหตุ OT ที่ใส่ไว้ใน plan.notes — แยกออกมาเป็นตัวช่วยเพราะหน้าที่มีคอลัมน์ OT
 * ของตัวเองแล้ว (ดูสแกนหน้า) ต้องกรองชิปตัวนี้ทิ้งให้ตรงตัว ไม่ใช่เดาจากคำขึ้นต้น
 */
export const otNote = (hours) => `OT ${hours}`;
...
export function planOf(record) {
  ...
  //
  // otherNote เข้ากองนี้เฉพาะวันที่หยุด/ลา เพราะวันนั้นมันคือ "เหตุผลที่ไม่มา"
  // วันที่ยังต้องมาทำงาน มันเป็นแค่หมายเหตุ ลงกอง notes ทางเดียว — เมื่อก่อนลงทั้งสองกอง
  // ช่องสถานะเลยขึ้นชิปเดียวกันสองอัน (ชิปแดงของ reasons + ชิปเทาของ notes)
  const reasons = [];
  if (leaveNote) reasons.push(leaveText(leaveNote));
  if (unpaidLeave) reasons.push(leaveText(unpaidLeave));
  if (otherNote && isOff) reasons.push(otherNote);
  ...
  if (otHours > 0) notes.push(otNote(otHours));
  ...
  return {
    ...
    breakOut: range[0] || '',
    breakIn: range[1] || '',
    // ระยะเบรคที่อนุญาต — เก็บทั้งข้อความที่สาขากรอก (ไว้แสดงในตาราง) และจำนวนนาที (ไว้คิดเบรคสาย)
    breakText: text(record.breakTime),
    ...
    // กันซ้ำอีกชั้นเผื่อข้อมูลซ้ำกันเองในแถวเดียว (เช่น otherNote ถูกกรอกเป็นข้อความเดียวกับเหตุผลลา)
    // ชิปเดียวกันสองอันในช่องสถานะอ่านแล้วสับสน และ key ของ React ก็ชนกันด้วย
...
/**
 * รวมสาย (นาที) = เข้าสาย + เบรคสาย + ออกก่อน — ตัวเดียวกับคอลัมน์ "รวมสาย" ในหน้าดูสแกนหน้า
 * และเป็นยอดสายที่หน้ารายงานเงินเดือนใช้ · ทั้งสามช่องเทียบไม่ได้ (เช่น วันหยุด/วันลา) = null
 */
export function totalLateOf(d) {
