...
// ยกเว้น "พนักงาน" ที่ย้ายไปอยู่ฐาน narai_hr แล้ว — ดู HR_EMPLOYEE_TABLE ข้างล่าง
//
...
/*
 * ตารางพนักงานตัวจริง — อยู่คนละฐานกับตารางอื่นในไฟล์นี้ (ดู docs/schema-hr-employee.sql)
 *
 * เดิมรายชื่อพนักงานนอนอยู่สองที่บนเครื่อง SQL ตัวเดียวกัน: InventoryNarai.dbo.hr_employee
 * (ที่หน้ารายชื่อของ naraipizzeria อ่าน/เขียน) กับ narai_hr.dbo.hr_employee (ที่ตารางงาน/กะ
 * ของ Narai-branch ใช้ — dbo.hr_timesheet อ้าง hr_code ของฐานนี้) ทั้งสองที่แก้กันคนละรอบ
 * ชื่อ/สาขา/สถานะจึงเพี้ยนกันไปเรื่อย ๆ
 *
 * รวมเหลือตารางเดียวที่ narai_hr เพราะประวัติกะผูกกับ hr_code ของฐานนั้นอยู่แล้ว ย้ายกลับทางอื่น
 * ไม่ได้ ส่วน InventoryNarai.dbo.hr_employee เหลือไว้เป็นสำเนาสำรองเฉย ๆ ไม่มีใครอ่าน/เขียนแล้ว
 *
 * ต้องเขียนเป็นชื่อสามส่วนเพราะ pool ที่ยิงเข้ามาต่ออยู่กับ InventoryNarai — สองฐานอยู่ instance
 * เดียวกัน SQL Server จึงข้ามฐานให้ได้เอง ขอแค่ login ที่ใช้มีสิทธิ์ในฐาน narai_hr ด้วย
 * (ไม่มีสิทธิ์จะขึ้นว่า "The server principal ... is not able to access the database")
 * ตั้ง env HR_EMPLOYEE_TABLE ทับได้ทั้งฝั่งเครื่องโฮสต์และฝั่ง Vercel ถ้าวันหลังย้ายฐานอีก
 */
export const HR_EMPLOYEE_TABLE = process.env.HR_EMPLOYEE_TABLE || 'narai_hr.dbo.hr_employee';
...
export function createSheets({ q }) {
  ...
  async function saveEmployee(body) {
    ...
    const rows = await q(
      `UPDATE ${HR_EMPLOYEE_TABLE} SET ${sets.join(', ')}, updated_at = SYSDATETIME() WHERE hr_code = @hr_code;
