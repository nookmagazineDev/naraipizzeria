 ...
 *
 * แถวที่ไม่มีเลขมิเตอร์เลย (ค่าเช่าพื้นที่ · tel) ไม่มีจำนวนให้คูณ — ราคาที่กรอกคือยอดของเดือนนั้น
 * จึงเอาราคาเป็นผลรวมให้ ไม่งั้นหน้าเว็บที่มีแต่ช่องราคาจะบันทึกยอดไม่ได้เลย (ผลรวมเป็นค่าว่าง)
 * ต้องไม่มีทั้งเลขเริ่มต้นและสิ้นสุด แถวมิเตอร์ที่กรอกมาครึ่งเดียวจะได้ไม่กลายเป็นยอดเงิน
 */
function buildExpenseRow(month, branch, it) {
  ...
  const noMeter = start === null && end === null;
  const total = given !== null ? given
    : noMeter ? price
