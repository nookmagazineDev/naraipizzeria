...
async function getFromHost(path, { timeoutMs = HOST_TIMEOUT } = {}) {
  ...
  // 404 = ไปถึงเครื่องแล้วแต่ไม่มีทางนี้ให้เรียก — เกือบทุกครั้งคือ host-server ยังรันโค้ดเก่าอยู่
  // (node ถือโค้ดเวอร์ชันตอนที่มันเริ่ม git pull เฉย ๆ ไม่พอ) บอกให้ตรงจุดจะได้ไม่ไปไล่หาที่อื่น
  if (res.status === 404) {
    throw new Error(
      `host API ยังไม่มี /aoringo/${String(path).split('?')[0]} (HTTP 404) — host-server ที่เครื่องออฟฟิศ ` +
      'ยังรันโค้ดเก่าที่ไม่มีทางนี้ ที่เครื่องนั้นสั่ง git pull แล้ว start-narai.ps1 -Restart หนึ่งครั้ง ' +
      'ถ้าทางอื่นของ /aoringo/* ก็ 404 เหมือนกันแปลว่า tunnel หลุดหรือชี้ผิดเครื่อง');
  }
