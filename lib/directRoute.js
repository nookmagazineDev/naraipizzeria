...
export function explainHostError(err, { base, timeoutMs } = {}) {
  ...
  // ไม่ตอบใน 55 วิ ยังแปลได้อีกทางคือ "ตอบอยู่แต่ช้าเกิน" ซึ่งของหน้ายอดขายเจอบ่อยกว่า
  // (ตาราง Cpaid/Ctrans ยังไม่มี index ตามวันที่ → สแกนทั้งตาราง วันเดียวก็ 125 วิ)
  const slowHint =
    ' · ถ้าเป็นหน้ายอดขาย/รายละเอียดรายการ อีกสาเหตุคือฐานข้อมูลที่ร้านช้าเพราะยังไม่มี index ' +
    'ตามวันที่ (แก้ด้วย scripts\\fix-sales-index.ps1 ที่เครื่องออฟฟิศ) ' +
    'ระหว่างนี้เลือกทีละสาขาและช่วงวันสั้น ๆ จะดึงได้';

  if (err?.name === 'TimeoutError' || /aborted due to timeout|operation was aborted/i.test(msg)) {
    return new Error(`เรียก ${where} ไม่ตอบ${secs}${officeHint}${slowHint}`);
  }
...
/** ตัด tag/ช่องว่างส่วนเกินออกจาก body ที่ไม่ใช่ JSON (หน้า error ของ tunnel เป็น HTML) */
...
/**
 * อ่าน body ของ response ที่ไม่ ok แล้วแปลว่าเกิดอะไรขึ้นจริง ๆ
 *
 * ของเดิมโยนแค่ `Upstream HTTP 500` ทิ้ง body ที่ host API อุตส่าห์ส่ง error จริงมาให้
 * บนหน้าเว็บจึงเห็นแค่ "ไม่สามารถดึงข้อมูลได้: Upstream HTTP 500" ซึ่งบอกไม่ได้เลยว่า
 * SQL ล้ม · คำสั่งช้าเกินจนถูกตัด · หรือ tunnel เป็นตัวที่ตอบ error page มา
 */
export async function explainUpstreamError(upstream, { base } = {}) {
  // ข้อความที่แปลแล้วอาจมีคำอย่าง "Failed to connect"/ECONNREFUSED ของฝั่ง SQL ติดมาด้วย
  // ซึ่งไปตรงกับตัวจับของ explainHostError พอดี — ติดธงไว้ให้ผู้เรียกข้ามการแปลรอบสอง
  // ไม่งั้นจะกลายเป็น "เรียก host API ไม่ได้" ทั้งที่ host API ตอบกลับมาแล้ว
  const explained = (msg) => Object.assign(new Error(msg), { explained: true });
  const status = upstream?.status || 0;
  const where = base ? `host API (${base})` : 'host API';
  let raw = '';
  try { raw = (await upstream.text()).slice(0, 4000); } catch { /* อ่าน body ไม่ได้ก็ช่างมัน */ }

  // host-server คืน {"error":"..."} — เอาข้อความในนั้นมาใช้ ไม่ใช่ทั้งก้อน JSON
  let detail = '';
  try {
    const j = JSON.parse(raw);
    detail = String(j?.error || j?.message || '').trim();
  } catch {
    detail = stripHtml(raw).slice(0, 300);
  }

  // mssql ตัดคำสั่งที่ทำงานนานเกิน requestTimeout ทิ้งแล้วโยนข้อความนี้ออกมา
  // = คำสั่งไม่ได้ผิด แต่ฐานข้อมูลที่ร้านช้าเกินกว่าจะดึงทัน (ยังไม่มี index ตามวันที่)
  const timeoutMatch = /Timeout:?\s*Request failed to complete in (\d+)\s*ms/i.exec(detail);
  if (timeoutMatch) {
    const secs = Math.round(Number(timeoutMatch[1]) / 1000);
    return explained(
      `SQL Server ที่ร้านดึงข้อมูลไม่เสร็จใน ${secs} วิ เลยถูกตัดทิ้ง — ` +
      'สาเหตุคือตาราง Cpaid/Ctrans ยังไม่มี index ตามวันที่ ต้องสแกนทั้งตารางทุกครั้ง ' +
      '(แก้ถาวร: รัน scripts\\fix-sales-index.ps1 ที่เครื่องออฟฟิศตอนปิดร้าน) ' +
      '· ระหว่างนี้เลือก "สาขา" ทีละสาขาแทน "ทุกสาขา" และดึงทีละไม่กี่วัน จะเร็วกว่ามาก'
    );
  }

  // host-server ต่อ SQL ไม่ติด (ฐานไม่ขึ้น/รหัสเปลี่ยน/service ไม่ได้รัน)
  if (isUnreachable(detail)) {
    return explained(
      `${where} ต่อ SQL Server ที่เครื่องออฟฟิศไม่ได้: ${detail} ` +
      '— ที่เครื่องนั้นเช็กว่า service SQL Server รันอยู่ไหม แล้วรีสตาร์ท host-server ด้วย ' +
      'start-narai.ps1 -Restart'
    );
  }

  // 502/503/504 ที่ไม่มี body ของเราเลย = tunnel ตอบเอง (host-server ไม่ได้รันอยู่ปลายทาง)
  if ([502, 503, 504].includes(status) && !detail) {
    return explained(
      `${where} ตอบ HTTP ${status} — tunnel ต่อเข้าเครื่องออฟฟิศไม่ได้ ` +
      '(ที่เครื่องนั้นลองเปิด http://localhost:14365/ping ถ้าได้แปลว่า tunnel เป็นตัวที่หลุด)'
    );
  }

  return explained(`${where} ตอบ HTTP ${status}${detail ? `: ${detail}` : ''}`);
}
