...
// จะย้ายเวลาทำงาน 1 ชม. ไปเป็น OT ให้อัตโนมัติ (ดู DAY9_OT_MINUTES)
//
...
export function summarizeSalary(dailyRows, { holidays = [] } = {}) {
  ...
  for (const row of rows) {
    ...
    // ย้ายได้ไม่เกินเวลาที่ทำจริง — วันที่ทำงานสั้นกว่า 1 ชม. ย้ายเท่าที่มี ไม่ให้เวลาทำงานติดลบ
    const day9Ot = isDay9(plan?.empType) ? Math.min(DAY9_OT_MINUTES, worked) : 0;
