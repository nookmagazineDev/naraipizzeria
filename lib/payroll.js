...
//
// พนักงานประเภท DAY9 ลงตารางวันละ 9 ชม. โดยชั่วโมงสุดท้ายนับเป็น OT — ทุกวันที่มาทำงาน
// ครบ 8 ชม. ขึ้นไป จะย้ายเวลาทำงาน 1 ชม. ไปเป็น OT ให้อัตโนมัติ (ดู DAY9_OT_MINUTES)
//
...
import { totalLateOf } from './attendance';
...
/** ตัวพิมพ์ใหญ่ ตัดช่องว่าง/จุด/ขีด/ทับ — 'day 9' · 'DAY-9' · 'Day/9' ได้ 'DAY9' เหมือนกันหมด */
const normType = (v) => text(v).toUpperCase().replace(/[\s.\-/]/g, '');

/**
 * ประเภทการจ่ายของพนักงาน — อ่านจากช่อง "สถานะ" ในตารางงาน (DAY / DAY9 / F/T / P/T)
...
/**
 * DAY9 — ลงตารางวันละ 9 ชั่วโมง โดยตกลงกันว่าชั่วโมงสุดท้ายของทุกวันนับเป็น OT
 *
 * ทุก "วันที่มาทำงานจริง" จึงย้ายเวลาทำงาน 1 ชั่วโมงไปเป็น OT — เวลาทำงานลด OT เพิ่ม
 * ยกเว้นวันที่ทำงาน (หักสายแล้ว) ไม่ถึง 8 ชม. — ยังทำไม่ครบเวลาปกติ จึงไม่มีชั่วโมงเกินให้นับเป็น OT
 * วันนั้นเวลาทำงานทั้งหมดเป็นเวลาทำงานปกติ (ดู DAY9_MIN_WORK_MINUTES)
 * จำนวนวันเท่าเดิม ช่อง "วันทำงาน" ท้ายฟอร์มจึงไม่เปลี่ยน (DAY9 ยังคิดเป็นรายวันเหมือน DAY)
 *
 * นับเฉพาะวันที่มาทำงาน — รวมวันทำงาน นข ด้วย (มาทำงานเหมือนกัน) แต่ไม่นับวันลา/วันหยุด
 * ที่ยังได้ค่าแรง เพราะวันพวกนั้นไม่ได้มาทำงานจึงไม่มีชั่วโมงให้ย้าย
 *
 * OT ที่สาขาลงไว้ในตารางงานยังบวกเพิ่มตามปกติ ตัวนี้เป็นการบวกทับเข้าไปอีกชั้น
 */
const DAY9_OT_MINUTES = 60;
/** DAY9 ต้องทำงาน (หักสายแล้ว) อย่างน้อยเท่านี้ในวันนั้น ถึงจะย้าย 1 ชม. ไปเป็น OT */
const DAY9_MIN_WORK_MINUTES = 8 * 60;

/** พนักงานคนนี้เป็นประเภท DAY9 ไหม (เทียบหลังตัดช่องว่าง/เครื่องหมายแล้ว) */
export const isDay9 = (empType) => normType(empType) === 'DAY9';
...
const blankPerson = () => ({
  ...
  position: '',
  empType: '',
  hasPlan: false,     // มีอย่างน้อยหนึ่งวันที่อยู่ในตารางงาน (ดูหมายเหตุที่ summarizeSalary)
  ...
  dayRows: [],            // แถวรายวันของคนนี้ (ไว้กางดูตารางงาน+เวลาสแกนตอนกดที่ชื่อ)
  day9Days: 0,            // จำนวนวันที่ย้ายชั่วโมงเป็น OT ตามกติกา DAY9
  day9OtHours: 0,         // ชั่วโมงที่ย้ายจากเวลาทำงานไปเป็น OT (รวมอยู่ใน otHours แล้ว)
 ...
 *
 * แต่ละคนมีธง hasPlan บอกว่ามีวันในตารางงานไหม — hasPlan = false คือรหัสที่มีแต่การสแกน
 * จับคู่กับตารางงานไม่ติดเลย (รหัสสองระบบไม่ตรงกัน และเครื่องสแกนไม่มีชื่อให้จับคู่ต่อ)
 * ซึ่งมักเป็นคนเดียวกับแถวที่มีชื่ออยู่แล้ว หน้ารายงานจึงคัดออกไม่ให้ซ้ำ แล้วขึ้นหมายเหตุแทน
 * ไฟล์นี้ไม่กรองให้เอง เพื่อให้ฝั่งที่เรียกใช้ยังนับได้ว่าตัดอะไรออกไปบ้าง
 */
export function summarizeSalary(dailyRows, { holidays = [] } = {}) {
  ...
  for (const row of rows) {
    ...
    // วันทำงาน — สายคิดจากเวลาสแกนเทียบตารางงาน ใช้ยอด "รวมสาย" (เข้าสาย + เบรคสาย + ออกก่อน)
    // ตัวเดียวกับคอลัมน์รวมสายในหน้าดูสแกนหน้า
    const late = num(totalLateOf(row));
    ...
    // DAY9: ย้ายเวลาทำงาน 1 ชม. ของวันนี้ไปเป็น OT (ดูหมายเหตุที่ DAY9_OT_MINUTES)
    // วันที่ทำงานไม่ถึง 8 ชม. (หักสายแล้ว) ไม่คิด OT เพิ่ม — เวลาที่ทำทั้งหมดเป็นเวลาทำงานปกติ
    const day9Ot = isDay9(plan?.empType) && worked >= DAY9_MIN_WORK_MINUTES ? DAY9_OT_MINUTES : 0;
    const workedNet = worked - day9Ot;
    if (day9Ot > 0) {
      p.day9Days += 1;
      p.day9OtHours += day9Ot / 60;
      p.otHours += day9Ot / 60;
    }
    ...
    if (holidaySet.has(row.date)) {
      ...
      p.holidayWorkMinutes += workedNet;
    } else {
      p.workDays += 1;
      p.lateMinutes += late;
      p.workMinutes += workedNet;
    }
  ...
  return [...byPerson.values()]
    .map((p) => ({
      ...
      dayRows: [...p.dayRows].sort((a, b) => String(a.date).localeCompare(String(b.date))),
      otHours: round2(p.otHours),
      day9OtHours: round2(p.day9OtHours),
