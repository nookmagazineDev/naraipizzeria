 ...
 * เพิ่งต่อตรงไม่ติดเมื่อครู่ก็ไม่ต้องไปนั่งรอหมดเวลาซ้ำอีก (ดู lib/directRoute.js)
 */
export async function tableIsReady() {
  if (!hasDirectDb() || directDown()) return false;
  try {
    await runQuery('SELECT TOP 1 1 AS ok FROM dbo.app_user');
    clearDirectDown();
    return true;
  } catch (err) {
    if (isUnreachable(err.message)) markDirectDown();
    return false;
  }
}
...
export async function authenticate(username, password) {
  ...
  let dbUnreachable = '';   // ข้อความ error ตอนไปไม่ถึง SQL — ไว้บอกสาเหตุจริงท้ายฟังก์ชัน
  ...
  // เพิ่งต่อตรงไม่ติดเมื่อครู่ = ข้ามไปเลย ไม่ต้องให้คนกรอกรหัสแล้วนั่งรอหมดเวลาอีก 8 วิ
  // แต่ต้องจำไว้ว่า "ข้ามเพราะต่อไม่ถึง" ไม่ใช่ "ไม่มีชื่อนี้" ไม่งั้นข้อความท้ายฟังก์ชันจะโกหก
  if (hasDirectDb() && directDown()) {
    dbUnreachable = 'ต่อ SQL Server ตรงไม่ติดเมื่อครู่ จึงยังไม่ลองใหม่ในช่วงนี้';
  } else if (hasDirectDb()) {
    try {
      ...
      // "ไปไม่ถึงเครื่อง SQL" ต้องนับเป็น "ยังไม่มีคลังผู้ใช้" เหมือนกับตอนที่ยังไม่ได้สร้างตาราง
      // ไม่งั้นวันที่ร้านไม่ได้เปิดพอร์ตออกเน็ต (ซึ่งเป็นสภาพปกติ — ดู lib/directRoute.js)
      // จะไม่มีใครเข้าระบบได้เลยแม้แต่ผู้ดูแล ทั้งที่หน้าล็อกอินขึ้นแถบบอกว่าเข้าด้วยบัญชีตั้งต้นได้
      const unreachable = isUnreachable(err.message);
      if (unreachable) markDirectDown();
      if (!unreachable && !isMissingTable(err.message)) {
        console.error('auth: อ่านคลังผู้ใช้ไม่ได้:', err.message);
        return { ok: false, message: `ตรวจสอบผู้ใช้กับฐานข้อมูลไม่ได้ (${err.message})` };
      }
      if (unreachable) dbUnreachable = err.message;
      // ต่อฐานไม่ถึง/ยังไม่ได้สร้างตาราง — ตกไปใช้โหมดตั้งค่าด้านล่าง
    }
  }
  ...
  // ต่อฐานไม่ถึง แล้วชื่อที่กรอกก็ไม่ใช่บัญชีสำรอง — บอกสาเหตุจริง อย่าตอบ "รหัสผิด"
  // ให้เขานั่งลองรหัสซ้ำ ๆ ทั้งที่ปัญหาอยู่ที่ทางไปฐาน ไม่ใช่ที่รหัสของเขา
  if (dbUnreachable) {
    return {
      ok: false,
      message: 'ตอนนี้ยังต่อฐานข้อมูลผู้ใช้ไม่ได้ จึงตรวจรหัสผ่านของบัญชีนี้ไม่ได้ ' +
        `(${dbUnreachable}) — เข้าด้วยบัญชีผู้ดูแลสำรองไปก่อน แล้วดู docs/login-permissions.md`,
    };
  }
  return WRONG;
}
